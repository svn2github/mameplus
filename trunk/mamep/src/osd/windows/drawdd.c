...
int drawdd_init(win_draw_callbacks *callbacks)
{
	...
	if (dllhandle == NULL)
	{
		verbose_printf(_WINDOWS("DirectDraw: Unable to access ddraw.dll\n"));
		return 1;
	}
	...
	{
		verbose_printf(_WINDOWS("DirectDraw: Unable to find DirectDrawCreateEx\n"));
	...
	{
		verbose_printf(_WINDOWS("DirectDraw: Unable to find DirectDrawEnumerateExA\n"));
		FreeLibrary(dllhandle);
		dllhandle = NULL;
		return 1;
	}
	...
	verbose_printf(_WINDOWS("DirectDraw: Using DirectDraw 7\n"));
	return 0;
}
...
error:
	...
	fprintf(stderr, _WINDOWS("Unable to initialize DirectDraw.\n"));
	return 1;
}
...
static int drawdd_window_draw(win_window_info *window, HDC dc, int update)
{
	...
	{
		verbose_printf(_WINDOWS("DirectDraw: Lost surfaces; deleting and retrying next frame\n"));
		ddraw_delete_surfaces(window);
		return 1;
	}
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X locking blit surface\n"), (int)result);
		return 1;
	}
	...
	if (result != DD_OK) verbose_printf(_WINDOWS("DirectDraw: Error %08X unlocking blit surface\n"), (int)result);
	...
	if ((video_config.waitvsync || video_config.syncrefresh) && video_config.throttle && (!window->fullscreen || dd->back == NULL))
	{
		...
		if (result != DD_OK) verbose_printf(_WINDOWS("DirectDraw: Error %08X waiting for VBLANK\n"), (int)result);
	}
...
static int ddraw_create(win_window_info *window)
{
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X during DirectDrawCreateEx call\n"), (int)result);
		goto error;
	}
	...
	if (verify == 2)
	{
		fprintf(stderr, _WINDOWS("DirectDraw: Error - Device does not meet minimum requirements for DirectDraw rendering\n"));
		goto error;
	}
	if (verify == 1)
		verbose_printf(_WINDOWS("DirectDraw: Warning - Device may not perform well for DirectDraw rendering\n"));
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X during IDirectDraw7_SetCooperativeLevel(FOCUSWINDOW) call\n"), (int)result);
		goto error;
	}
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X during IDirectDraw7_SetCooperativeLevel(DEVICEWINDOW) call\n"), (int)result);
		goto error;
	}
	...
	if (window->fullscreen && video_config.switchres)
	{
		...
		if (result != DD_OK)
		{
			verbose_printf(_WINDOWS("DirectDraw: Error %08X attempting to set video mode %dx%d@%d call\n"), (int)result, dd->width, dd->height, dd->refresh);
			goto error;
		}
	}
...
static int ddraw_create_surfaces(win_window_info *window)
{
	...
	if (window->fullscreen && video_config.triplebuf)
	{
		...
		if (result != DD_OK)
		{
			verbose_printf(_WINDOWS("DirectDraw: Error %08X getting attached back surface\n"), (int)result);
			goto error;
		}
	}
	...
	if (window->fullscreen)
	{
		...
		if (brightness != 1.0f || contrast != 1.0f || gamma != 1.0f)
		{
			...
			if (result != DD_OK)
			{
				fprintf(stderr, _WINDOWS("DirectDraw: Warning - device does not support full screen gamma correction.\n"));
				dd->gamma = NULL;
			}
			...
			if (dd->gamma != NULL)
			{
				...
				if (result != DD_OK)
					verbose_printf(_WINDOWS("DirectDraw: Error %08X attempting to set gamma correction.\n"), (int)result);
			}
		}
	}
...
static int ddraw_verify_caps(dd_info *dd)
{
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X during IDirectDraw7_GetCaps call\n"), (int)result);
		return 1;
	}
	...
	{
		verbose_printf(_WINDOWS("DirectDraw: Warning - Device does not support hardware stretching\n"));
		retval = 1;
	}
...
static HRESULT create_surface(dd_info *dd, DDSURFACEDESC2 *desc, IDirectDrawSurface7 **surface, const char *type)
{
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X creating %s surface\n"), (int)result, type);
		return result;
	}
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X getting %s surface desciption\n"), (int)result, type);
	...
	verbose_printf(_WINDOWS("DirectDraw: %s surface created: %dx%dx%d (R=%08X G=%08X B=%08X)\n"),
				type,
				(int)desc->dwWidth,
				(int)desc->dwHeight,
...
static int create_clipper(win_window_info *window)
{
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X creating clipper\n"), (int)result);
		return 1;
	}
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X setting clipper hwnd\n"), (int)result);
		return 1;
	}
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X setting clipper on primary surface\n"), (int)result);
		return 1;
	}
	return 0;
}
...
static void compute_blit_surface_size(win_window_info *window)
{
	...
	if (newwidth != dd->blitwidth || newheight != dd->blitheight)
	{
		// force some updates
		update_outer_rects(dd);
		verbose_printf(_WINDOWS("DirectDraw: New blit size = %dx%d\n"), newwidth, newheight);
	}
...
static void blit_to_primary(win_window_info *window, int srcwidth, int srcheight)
{
	...
	if (dd->clearouter != 0)
	{
		...
		if (dest.left > outer.left)
		{
			...
			if (result != DD_OK) verbose_printf(_WINDOWS("DirectDraw: Error %08X clearing the screen\n"), (int)result);
		}
	...
	if (result != DD_OK) verbose_printf(_WINDOWS("DirectDraw: Error %08X blitting to the screen\n"), (int)result);
...
static int config_adapter_mode(win_window_info *window)
{
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error getting identifier for device\n"));
		return 1;
	}
	verbose_printf(_WINDOWS("DirectDraw: Configuring device %s\n"), identifier.szDescription);
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("DirectDraw: Error %08X getting current display mode\n"), (int)result);
		IDirectDraw7_Release(dd->ddraw);
		return 1;
	}
...
static void get_adapter_for_monitor(dd_info *dd, win_monitor_info *monitor)
{
	...
	if (result != DD_OK) verbose_printf(_WINDOWS("DirectDraw: Error %08X during DirectDrawEnumerateEx call\n"), (int)result);
...
static void pick_best_mode(win_window_info *window)
{
	...
	// enumerate the modes
	verbose_printf(_WINDOWS("DirectDraw: Selecting video mode...\n"));
	result = IDirectDraw7_EnumDisplayModes(dd->ddraw, DDEDM_REFRESHRATES, NULL, &einfo, enum_modes_callback);
	if (result != DD_OK) verbose_printf(_WINDOWS("DirectDraw: Error %08X during EnumDisplayModes call\n"), (int)result);
	verbose_printf(_WINDOWS("DirectDraw: Mode selected = %4dx%4d@%3dHz\n"), dd->width, dd->height, dd->refresh);
}
