...
void renderer::set_wrap(D3DTEXTUREADDRESS wrap)
{
	if (wrap != m_last_wrap)
	{
		...
		if (result != D3D_OK) mame_printf_verbose(_WINDOWS("Direct3D: Error %08X during device set_texture_stage_state call\n"), (int)result);
		result = (*d3dintf->device.set_texture_stage_state)(m_device, 0, (D3DTEXTURESTAGESTATETYPE)D3DTSS_ADDRESSV, wrap ? D3DTADDRESS_WRAP : D3DTADDRESS_CLAMP);
		if (result != D3D_OK) mame_printf_verbose(_WINDOWS("Direct3D: Error %08X during device set_texture_stage_state call\n"), (int)result);
		result = (*d3dintf->device.set_texture_stage_state)(m_device, 1, (D3DTEXTURESTAGESTATETYPE)D3DTSS_ADDRESSU, wrap ? D3DTADDRESS_WRAP : D3DTADDRESS_CLAMP);
		if (result != D3D_OK) mame_printf_verbose(_WINDOWS("Direct3D: Error %08X during device set_texture_stage_state call\n"), (int)result);
		result = (*d3dintf->device.set_texture_stage_state)(m_device, 1, (D3DTEXTURESTAGESTATETYPE)D3DTSS_ADDRESSV, wrap ? D3DTADDRESS_WRAP : D3DTADDRESS_CLAMP);
		if (result != D3D_OK) mame_printf_verbose(_WINDOWS("Direct3D: Error %08X during device set_texture_stage_state call\n"), (int)result);
	}
}
...
int renderer::device_test_cooperative()
{
	...
	if (result == D3DERR_DEVICENOTRESET)
	{
		...
		{
			mame_printf_verbose(_WINDOWS("Direct3D: failed to recreate HLSL resources for device; failing permanently\n"));
			device_delete();
			return 1;
		}
	}
...
void texture_info::set_data(const render_texinfo *texsource, UINT32 flags)
{
	...
	if (result != D3D_OK)
	{
		mame_printf_verbose(_WINDOWS("Direct3D: Error %08X during texture unlock_rect call\n"), (int)result);
	}
