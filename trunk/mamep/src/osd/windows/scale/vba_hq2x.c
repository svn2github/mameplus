//ref: http://vba-rerecording.googlecode.com/svn/trunk/src/hq2x.cpp
/*
...
static void hq2xS_16_def(u16* dst0, u16* dst1, const u16* src0, const u16* src1, const u16* src2, unsigned count)
{
  ...
  for(i=0;i<count;++i) {
	...
	for(j = 0 ; j < 9 ; j++)
	{
		...
		bright = r+r+r + g+g+g + b+b;
		if(bright > maxBright) maxBright = bright;
		if(bright < minBright) minBright = bright;

		brightArray[j] = bright;
	}
	diffBright = ((maxBright - minBright) * 7) >> 4;
	if(diffBright > 7)
	{
