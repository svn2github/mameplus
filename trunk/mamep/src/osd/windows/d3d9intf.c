...
d3d *drawd3d9_init(void)
{
	...
	if (dllhandle == NULL)
	{
		verbose_printf(_WINDOWS("Direct3D: Unable to access d3d9.dll\n"));
		return NULL;
	}
	...
	{
		verbose_printf(_WINDOWS("Direct3D: Unable to find Direct3DCreate9\n"));
	...
	if (d3d9 == NULL)
	{
		verbose_printf(_WINDOWS("Direct3D: Error attempting to initialize Direct3D9\n"));
		FreeLibrary(dllhandle);
		dllhandle = NULL;
		return NULL;
	}
	...
	verbose_printf(_WINDOWS("Direct3D: Using Direct3D 9\n"));
	return d3dptr;
}
