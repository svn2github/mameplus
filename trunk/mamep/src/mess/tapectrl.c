...
static void menu_tape_control_populate(running_machine *machine, ui_menu *menu, tape_control_menu_state *menustate)
{
	...
	if (image_exists(menustate->device))
	{
		...
		/* pause or stop */
		ui_menu_item_append(menu, _("Pause/Stop"), NULL, 0, TAPECMD_STOP);

		/* play */
		ui_menu_item_append(menu, _("Play"), NULL, 0, TAPECMD_PLAY);

		/* record */
		ui_menu_item_append(menu, _("Record"), NULL, 0, TAPECMD_RECORD);

		/* rewind */
		ui_menu_item_append(menu, _("Rewind"), NULL, 0, TAPECMD_REWIND);

		/* fast forward */
		ui_menu_item_append(menu, _("Fast Forward"), NULL, 0, TAPECMD_FAST_FORWARD);
	}
	else
	{
		/* no tape loaded */
		ui_menu_item_append(menu, _("No Tape Image loaded"), NULL, flags, NULL);
	}
}
