...
void ui_mess_main_menu_populate(running_machine *machine, ui_menu *menu)
{
	...
	for (port = machine->portlist.head; port != NULL; port = port->next)
	{
  	...
  	/* add image info menu */
	ui_menu_item_append(menu, _("Image Information"), NULL, 0, (void*)ui_mess_menu_image_info);

  	/* add image info menu */
	ui_menu_item_append(menu, _("File Manager"), NULL, 0, (void*)ui_mess_menu_file_manager);
	...
	if (device_list_first(&machine->config->devicelist, CASSETTE))
  	...
  	if (has_keyboard && inputx_can_post(machine))
		ui_menu_item_append(menu, _("Keyboard Mode"), NULL, 0, (void*)ui_mess_menu_keyboard_mode);
}
