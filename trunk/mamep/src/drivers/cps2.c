...
static ADDRESS_MAP_START( phoenix_writemem, ADDRESS_SPACE_PROGRAM, 16 )
	AM_RANGE(0x000000, 0x3fffff) AM_WRITE(MWA16_ROM)				/* ROM */
	AM_RANGE(0x618000, 0x619fff) AM_WRITE(qsound_sharedram1_w)		/* Q RAM */
	AM_RANGE(0x662000, 0x662001) AM_WRITE(MWA16_RAM)				/* Network adapter related, accessed in SSF2TB */
	AM_RANGE(0x662008, 0x662009) AM_WRITE(MWA16_RAM)				/* Network adapter related, accessed in SSF2TB (not sure if this port is write only yet)*/
	...
	AM_RANGE(0x804100, 0x8041ff) AM_WRITE(cps1_output_w) AM_BASE(&cps1_output) AM_SIZE(&cps1_output_size)  /* Output ports */
	AM_RANGE(0x900000, 0x92ffff) AM_WRITE(cps1_gfxram_w) AM_BASE(&cps1_gfxram) AM_SIZE(&cps1_gfxram_size)
	AM_RANGE(0xff0000, 0xffffef) AM_WRITE(MWA16_RAM)				/* RAM */
	AM_RANGE(0xfffff0, 0xfffffb) AM_WRITE(MWA16_RAM) AM_BASE(&cps2_output) AM_SIZE(&cps2_output_size)	/* CPS2 output */
ADDRESS_MAP_END
...
static MACHINE_DRIVER_START( cps2 )
	...
	MDRV_CPU_ADD_TAG("main", M68000, 11800000)	//mamep: cps2 phoenix
...
static MACHINE_DRIVER_START( phoenix )
	...
	MDRV_CPU_PROGRAM_MAP(cps2_readmem,phoenix_writemem)
MACHINE_DRIVER_END
...
ROM_START( pzloop2 )
	ROM_REGION( CODE_SIZE, REGION_CPU1, 0 )      /* 68000 code */
	ROM_LOAD16_WORD_SWAP( "pl2e.03", 0x000000, 0x80000, CRC(3b1285b2) )
	ROM_LOAD16_WORD_SWAP( "pl2e.04", 0x080000, 0x80000, CRC(40a2d647) )
	ROM_LOAD16_WORD_SWAP( "pl2e.05", 0x100000, 0x80000, CRC(0f11d818) )
	ROM_LOAD16_WORD_SWAP( "pl2e.06", 0x180000, 0x80000, CRC(86fbbdf4) )

	ROM_REGION16_BE( CODE_SIZE, REGION_USER1, 0 )
	ROM_LOAD16_WORD_SWAP( "pl2ex.03", 0x000000, 0x80000, CRC(816cbbd9) SHA1(2167bfdf18b1d6253be233e4d5e5a958effbb854) )
	ROM_LOAD16_WORD_SWAP( "pl2ex.04", 0x080000, 0x80000, CRC(41c19742) SHA1(3da08184e944221d4e5584844b04af338aed0938) )
	ROM_LOAD16_WORD_SWAP( "pl2ex.05", 0x100000, 0x80000, CRC(fe0eb38b) SHA1(9ea5aa8a3e79596513b237caa00deb1771e22b19) )
	ROM_LOAD16_WORD_SWAP( "pl2ex.06", 0x180000, 0x80000, CRC(491919eb) SHA1(1c38b317d7f49e22af30d32e8d361233f91e02da) )
...
ROM_START( 1944d )
	ROM_REGION( CODE_SIZE, REGION_CPU1, 0 )      /* 68000 code */
	ROM_LOAD16_WORD_SWAP( "nffud.03", 0x000000, 0x80000, CRC(8a2279b4) SHA1(4a7388ec0164171eee520e027e205164dc743b8d) )
	ROM_LOAD16_WORD_SWAP( "nff.04",  0x080000, 0x80000, CRC(dba1c66e) SHA1(4764e77d4da5d19d9acded27df1e1bcba06b0fcf) )
	ROM_LOAD16_WORD_SWAP( "nffu.05", 0x100000, 0x80000, CRC(ea813eb7) SHA1(34e0175a5f22d08c3538369b4bfd077a7427a128) )
...
ROM_START( hsf2j )
	...
	ROM_REGION16_BE( CODE_SIZE, REGION_USER1, 0 )
	ROM_LOAD16_WORD_SWAP( "hs2jx.03", 0x000000, 0x80000, NO_DUMP )
	ROM_LOAD16_WORD_SWAP( "hs2jx.04", 0x080000, 0x80000, NO_DUMP )
...
ROM_START( sfz2aad )
	ROM_REGION( CODE_SIZE, REGION_CPU1, 0 )      /* 68000 code */
	ROM_LOAD16_WORD_SWAP( "szaad.03", 0x000000, 0x80000, CRC(3b256466) SHA1(a79476a7a0ba196386ee920ca943e41558a8d32f) )
...
GAME( 2001, pzloop2, 	0,       cps2, puzloop2, pzloop2, ROT0,   "Mitchell, distributed by Capcom", "Puzz Loop 2 (Euro 010302)", 0 )
...
/* Phoenix Edition on CPS-2 hardware by Raz */

GAME( 1993, ddtodd,   ddtod,   phoenix, ddtod,   cps2, ROT0,   "hack", "Dungeons & Dragons: Tower of Doom (Phoenix Edition Euro 940412)", 0 )
GAME( 1994, xmcotad,  xmcota,  phoenix, ssf2,    cps2, ROT0,   "hack", "X-Men: Children of the Atom (Phoenix Edition Euro 950105)", 0 )
GAME( 1994, avspd,    avsp,    phoenix, avsp,    cps2, ROT0,   "hack", "Alien vs. Predator (Phoenix Edition Euro 940520)", 0 )
GAME( 1994, ringdstd, ringdest,phoenix, ssf2,    cps2, ROT0,   "hack", "Ring of Destruction: Slammasters II (Phoenix Edition Euro 940902)", 0 )
GAME( 1995, nwarrd,   nwarr,   phoenix, ssf2,    cps2, ROT0,   "hack", "Night Warriors: Darkstalkers' Revenge (Phoenix Edition US 950406)", 0 )
GAME( 1995, sfad,     sfa,     phoenix, ssf2,    cps2, ROT0,   "hack", "Street Fighter Alpha: Warriors' Dreams (Phoenix Edition Euro 950727)", 0 )
GAME( 1996, 19xxd,    19xx,    phoenix, 19xx,    cps2, ROT270, "hack", "19XX: The War Against Destiny (Phoenix Edition US 951207)", 0 )
GAME( 1996, ddsomud,  ddsom,   phoenix, ddtod,   cps2, ROT0,   "hack", "Dungeons & Dragons: Shadow over Mystara (Phoenix Edition US 960619)", 0 )
GAME( 1996, megamn2d, megaman2,phoenix, sgemf,   cps2, ROT0,   "hack", "Mega Man 2: The Power Fighters (Phoenix Edition US 960708)", 0 )
GAME( 1996, sfz2aad,  sfa2,    phoenix, ssf2,    cps2, ROT0,   "hack", "Street Fighter Zero 2 Alpha (Phoenix Edition Asia 960826)", 0 )
GAME( 1996, spf2xjd,  spf2t,   phoenix, 19xx,    cps2, ROT0,   "hack", "Super Puzzle Fighter II X (Phoenix Edition Japan 960531)", 0 )
GAME( 1996, xmvsfu1d, xmvsf,   phoenix, ssf2,    cps2, ROT0,   "hack", "X-Men Vs. Street Fighter (Phoenix Edition US 961004)", 0 )
GAME( 1997, batcird,  batcir,  phoenix, batcir,  cps2, ROT0,   "hack", "Battle Circuit (Phoenix Edition Euro 970319)", 0 )
