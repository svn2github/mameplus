...
static int val06kp,xr=4,xr2=3,lg=0;
static sound_stream *crbaloon_tone_stream;
static UINT32 crbaloon_tone_step;
static UINT32 crbaloon_tone_pos;
static sound_stream *crbaloon_tone_stream2;
static UINT32 crbaloon_tone_pos2;

static const char *crbaloon_sample_names[] =
{
	"*crbaloon",
	"pang.wav",
	"appear.wav",
...
{
	/* update the streams */
	stream_update(crbaloon_tone_stream, 0);

	crbaloon_tone_step = 0;

	if (data && data != 0xff)
	{
		double freq = (13630.0 / (256 - data) + (data>=0xea ? 13 : 0))*0.7;
...
static void crbaloon_tone_update(void *param, stream_sample_t **inputs, stream_sample_t **outputs, int len)
{
	...
	if (val06kp&0x04) xr = 4;
	else if (xr) xr--;
	if (crbaloon_tone_step && xr)
	{
		while (len-- > 0)
		{
	...
	if (val06kp&0x04) xr = 6;
}

static void crbaloon_tone_update2(void *param, stream_sample_t **inputs, stream_sample_t **outputs, int len)
{
	stream_sample_t *buffer = outputs[0];
	
	memset(buffer, 0, len * sizeof(stream_sample_t));

	if (val06kp&0x04) xr2 = 3;
	else if (xr2) xr2--;
	if (crbaloon_tone_step && xr2)
	{
		while (len-- > 0)
		{
			*buffer++ = crbaloon_tone_pos2 & 0x80000000 ? 32767 : -32768;
			crbaloon_tone_pos2 += crbaloon_tone_step*2;
		}
	}
	if (val06kp&0x04) xr2 = 5;
}
...
static void *crbaloon_sh_start2(int clock, const struct CustomSound_interface *config)
{
	crbaloon_tone_pos2 = 0;

	crbaloon_tone_stream2 = stream_create(0, 1, Machine->sample_rate, NULL, crbaloon_tone_update2);

	return auto_malloc(1);
}

struct Samplesinterface crbaloon_samples_interface =
{
	2,
	crbaloon_sample_names
};
...
struct CustomSound_interface crbaloon_custom_interface2 =
{
	crbaloon_sh_start2
};
...
static MACHINE_DRIVER_START( crbaloon )
	...
	MDRV_SOUND_ROUTE(ALL_OUTPUTS, "mono", 0.07)	// (xr+3 / 100.0)

	MDRV_SOUND_ADD(CUSTOM, 0)
	MDRV_SOUND_CONFIG(crbaloon_custom_interface2)
	MDRV_SOUND_ROUTE(ALL_OUTPUTS, "mono", 0.06)	// (xr2+3 / 100.0)

	MDRV_SOUND_ADD(SAMPLES, 0)
	MDRV_SOUND_CONFIG(crbaloon_samples_interface)
