...
{
	printf(_(APPNAME " is an emulator: it reproduces, more or less faithfully, the behaviour of\n"
		 "several arcade machines. But hardware is useless without software, so an image\n"
		 "of the ROMs which run on that hardware is required. Such ROMs, like any other\n"
		 "commercial software, are copyrighted material and it is therefore illegal to\n"
		 "use them if you don't own the original arcade machine. Needless to say, ROMs\n"
		 "are not distributed together with " APPNAME ". Distribution of " APPNAME " together with ROM\n"
...
static int count_roms(const rom_entry *romp)
{
	...
	for (region = romp; region; region = rom_next_region(region))
		for (rom = rom_first_file(region); rom; rom = rom_next_file(rom))
			if (!ROM_GETBIOS(romp) || (ROM_GETBIOS(romp) == (this_bios+1))) /* alternate bios sets */
				count++;
...
static void handle_missing_file(rom_load_data *romdata, const rom_entry *romp)
{
	...
	{
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("OPTIONAL %-12s NOT FOUND\n"), ROM_GETNAME(romp));
		romdata->warnings++;
	}
	...
	{
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%-12s NOT FOUND (NO GOOD DUMP KNOWN)\n"), ROM_GETNAME(romp));
		romdata->warnings++;
	}
	...
	{
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%-12s NOT FOUND\n"), ROM_GETNAME(romp));
		romdata->errors++;
	}
}
...
static void dump_wrong_and_correct_checksums(rom_load_data* romdata, const char* hash, const char* acthash)
{
	...
	if (hash_data_has_info(hash, HASH_INFO_NO_DUMP) || hash_data_has_info(hash, HASH_INFO_BAD_DUMP))
		strcpy(chksum, _("UNKNOWN"));
	else
	hash_data_print(hash, found_functions, chksum);
	sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("    EXPECTED: %s\n"), chksum);
	...
	sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("       FOUND: %s\n"), chksum);
...
static void verify_length_and_hash(rom_load_data *romdata, const char *name, UINT32 explength, const char* hash)
{
	...
	{
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%-12s WRONG LENGTH (expected: %08x found: %08x)\n"), name, explength, actlength);
		romdata->warnings++;
	}
	...
	{
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%-12s ROM NEEDS REDUMP\n"),name);
		romdata->warnings++;
	}
}
...
static int process_rom_entries(rom_load_data *romdata, const rom_entry *romp)
{
	...
	while (!ROMENTRY_ISREGIONEND(romp))
	{
		...
		{
			if (!ROM_GETBIOS(romp) || (ROM_GETBIOS(romp) == (system_bios+1))) /* alternate bios sets */
			{
		...
		if (ROMENTRY_ISFILE(romp))
		{
			...
			if (!source)
			{
				if (chd_get_last_error() == CHDERR_UNSUPPORTED_VERSION)
					sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%-12s UNSUPPORTED CHD VERSION\n"), filename);
				else
					sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%-12s NOT FOUND\n"), filename);
				romdata->errors++;
				romp++;
				...
				if (!diff)
				{
					...
					if (err != CHDERR_NONE)
					{
						if (chd_get_last_error() == CHDERR_UNSUPPORTED_VERSION)
							...
							sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%-12s: CAN'T CREATE DIFF FILE\n"), filename);
						romdata->errors++;
						romp++;
						continue;
					}
					...
					if (!diff)
					{
						if (chd_get_last_error() == CHDERR_UNSUPPORTED_VERSION)
							...
							sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%-12s: CAN'T OPEN DIFF FILE\n"), filename);
						romdata->errors++;
						romp++;
						continue;
					}
