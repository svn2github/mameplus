...
#ifdef USE_SCALE_EFFECTS
static LPDIRECTDRAWSURFACE7 scale_surface;
#endif /* USE_SCALE_EFFECTS */
...
static UINT8 win_d3d_swapxy;
static UINT8 win_d3d_flipx;
static UINT8 win_d3d_flipy;
...
static int win_d3d_test_hardware_caps(void)
{
	...
	if (win_d3d_use_scanlines)
	{
		...
		{
			fprintf(stderr, _WINDOWS("Error: 3D hardware can't render scanlines (no support for multi-texturing)\n"));
			return 1;
		}
		...
		if (d3d_device_desc.dwTextureOpCaps & D3DTEXOPCAPS_ADDSMOOTH)
		{
			...
			verbose_printf(_WINDOWS("Direct3D: Warning: using fall-back method for scanline blending.\n"));
	}
	...
	switch (win_d3d_use_rgbeffect)
	{
		...
		case 1:
		{
			...
			{
				verbose_printf(_WINDOWS("Direct3D: Error: RGB effects (multiply mode) not supported\n"));
				return 1;
			}
			break;
		}
		...
		case 2:
		{
			...
			{
				verbose_printf(_WINDOWS("Direct3D: Error: RGB effects (multiply & add mode) not supported\n"));
				return 1;
			}
			break;
		}
...
static void adjust_prescale(int width, int height)
{
...
#ifdef USE_SCALE_EFFECTS
	current_prescalex /= effect_min_xscale * scale_effect.xsize;
	...
	// adjust if software scaling is used
#ifdef USE_SCALE_EFFECTS
	current_prescaley /= effect_min_yscale * scale_effect.ysize;
#else /* USE_SCALE_EFFECTS */
...
int win_d3d_init(int width, int height, int depth, int attributes, double aspect, const win_effect_data *effect)
{
	...
	if (ddraw_dll7 == NULL)
	{
		fprintf(stderr, _WINDOWS("Error importing ddraw.dll\n"));
		goto error_handling;
	}
	...
	{
		fprintf(stderr, _WINDOWS("Error importing DirectDrawCreateEx() function\n"));
		goto error_handling;
	}
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error creating DirectDraw7: %08x\n"), (UINT32)result);
		goto error_handling;
	}
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error creating Direct3D7: %08x\n"), (UINT32)result);
		goto error_handling;
	}
	...
	// print initialisation message and available video memory
	{
		...
		verbose_printf(_WINDOWS("Initialising DirectDraw & Direct3D 7 blitter"));
		if (IDirectDraw7_GetAvailableVidMem(ddraw7, &caps, &mem_total, &mem_free) == DD_OK)
		{
			verbose_printf(_WINDOWS(" (%.2lfMB video memory available)"), (double)mem_total / (1024 * 1024));
		}
		verbose_printf("\n");
	}
	...
	win_d3d_flipx		= blit_flipx;
	win_d3d_flipy		= blit_flipy;
	win_d3d_swapxy		= blit_swapxy;
#else
	win_d3d_flipx		= 0;
	win_d3d_flipy		= 0;
	win_d3d_swapxy		= 0;
...
#ifdef USE_SCALE_EFFECTS
	if (video_attributes & VIDEO_TYPE_VECTOR)
	...
	if (scale_effect.effect)
		verbose_printf(_WINDOWS("Using %s pre-scale effect\n"), scale_effect.name);
#endif /* USE_SCALE_EFFECTS */
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error creating Direct3D7 device: %08x\n"), (UINT32)result);
		goto error_handling;
	}
	...
	// print available video memory
	{
		...
		if (IDirectDraw7_GetAvailableVidMem(ddraw7, &caps, &mem_total, &mem_free) == DD_OK)
		{
			verbose_printf(_WINDOWS("Blitter initialisation complete (%.2lfMB video memory free)\n"), (double)mem_free / (1024 * 1024));
		}
	}
...
static double compute_mode_score(int width, int height, int depth, int refresh)
{
...
#ifdef USE_SCALE_EFFECTS
	target_width *= win_d3d_swapxy ? scale_effect.ysize : scale_effect.xsize;
	target_height *= win_d3d_swapxy ? scale_effect.xsize : scale_effect.ysize;
#endif /* USE_SCALE_EFFECTS */
	...
	{
		if (win_d3d_swapxy && win_default_constraints != CONSTRAIN_INTEGER_HEIGHT)
		{
...
static int set_resolution(void)
{
	...
	if (!win_window_mode && (win_switch_res || win_switch_bpp))
	{
		...
		if (best_refresh)
			verbose_printf(_WINDOWS("Best mode = %dx%dx%d @ %d Hz\n"), best_width, best_height, best_depth, best_refresh);
		else
			verbose_printf(_WINDOWS("Best mode = %dx%dx%d @ default Hz\n"), best_width, best_height, best_depth);
...
static int create_surfaces(void)
{
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("Error creating primary surface: %08x\n"), (UINT32)result);
		goto error_handling;
	}
	...
	// print out the good stuff
	{
		...
		if (win_window_mode)
			verbose_printf(_WINDOWS("Rendering to an off-screen surface and using blt()\n"));
		else
			verbose_printf(_WINDOWS("Using a %s buffer and pageflipping\n"), win_triple_buffer ? _WINDOWS("triple") : _WINDOWS("double"));

		print_surface_size(primary_surface, size);
		print_surface_colour(primary_surface, colour);

		verbose_printf(_WINDOWS("Primary surface created: %s (%s)\n"), size, colour);
	}
	...
	if (win_window_mode)
	{
		...
		if (result != DD_OK)
		{
			fprintf(stderr, _WINDOWS("Error creating back surface: %08x\n"), (UINT32)result);
			goto error_handling;
		}
	}
...
static int create_effects_surfaces(void)
{
	...
	if (win_d3d_use_rgbeffect)
	{
		...
		if (result != DD_OK)
		{
			fprintf(stderr, _WINDOWS("Error creating RGB effects surface: %08x\n"), (UINT32)result);
			goto error_handling;
		}

		{
			...
			verbose_printf(_WINDOWS("RGB effects surface created: %s\n"), size);
		}
	}
	...
	if (win_d3d_use_scanlines)
	{
		...
		for (i = 0; i < 2; i++)
		{
			...
			{
				fprintf(stderr, _WINDOWS("Error creating scanline surface[%d]: %08x\n"), i, (UINT32)result);
				goto error_handling;
			}
...

			{
				...
				verbose_printf(_WINDOWS("Scanline surface[%i] created: %s\n"), i, size);
			}
		}
	}
...
{
#ifdef USE_SCALE_EFFECTS
	if (!scale_check(15) && pixelformat->dwFlags == DDPF_RGB && pixelformat->DUMMYUNIONNAMEN(1).dwRGBBitCount == 16)
#else /* USE_SCALE_EFFECTS */
...
static int create_blit_surface(void)
{
	...
	while (!done)
	{
		...
		width = (win_d3d_swapxy ? max_height : max_width) * effect_min_xscale + 16;
		height = (win_d3d_swapxy ? max_width : max_height) * effect_min_yscale + 0;

		// reduce the width if needed
		if (width > win_d3d_swapxy ? primary_desc.dwHeight : primary_desc.dwWidth)
		{
		...
		if (height > win_d3d_swapxy ? primary_desc.dwWidth : primary_desc.dwHeight)
		{
...
#ifdef USE_SCALE_EFFECTS
	while (texture_width < (width - 16) * scale_effect.xsize)
		texture_width <<= 1;
	while (texture_height < (height - 0) * scale_effect.ysize)
		texture_height <<= 1;

	if (win_d3d_tex_manage && !scale_effect.effect && texture_width < width)
		texture_width <<= 1;
#else /* USE_SCALE_EFFECTS */
...
#ifdef USE_SCALE_EFFECTS
	if (scale_check(32) || !needs_6bpp_per_gun)
#else /* USE_SCALE_EFFECTS */
...
#ifdef USE_SCALE_EFFECTS
	if (!scale_effect.effect && win_d3d_tex_manage)
#else /* USE_SCALE_EFFECTS */
	...
	else
	{
		...
		blit_desc.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | (scale_effect.effect ? DDSCAPS_SYSTEMMEMORY : (DDSCAPS_3DDEVICE | DDSCAPS_VIDEOMEMORY));
#else /* USE_SCALE_EFFECTS */
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("Error creating blit surface: %08x\n"), (UINT32)result);
		goto error_handling;
	}
...
#endif /* USE_SCALE_EFFECTS */
	{
		...
		if (result != DD_OK)
		{
			verbose_printf(_WINDOWS("Error creating texture surface: %08x\n"), (UINT32)result);
			goto error_handling;
		}
	}
	...
	if ((win_d3d_swapxy ? max_width : max_height) > 256)
		current_prescaley++;
	...
	if (win_d3d_use_prescale || win_d3d_use_feedback)
	{
		...
		{
			verbose_printf(_WINDOWS("Warning: required texture size too large for prescale, disabling prescale\n"));
		...
		if (result == DD_OK)
		{
			...
			if (IDirect3DDevice7_GetRenderTarget(d3d_device7, &previous_target) == DD_OK)
			{
				if (IDirect3DDevice7_SetRenderTarget(d3d_device7, preprocess_surface, 0) == DD_OK)
					...
					verbose_printf(_WINDOWS("Error: feedback or prescale effects not supported.\n"));
...
#endif
		{
			verbose_printf(_WINDOWS("Error creating preprocess surface: %08x\n"), (UINT32)result);
			goto error_handling;
		}
	}
	...
	if (scale_effect.effect && !win_d3d_tex_manage)
	{
		DDSURFACEDESC2 desc = blit_desc;

		// fill in the differences
		desc.dwWidth = (width - 16) * scale_effect.xsize;
		desc.dwHeight = height * scale_effect.ysize + scale_effect.ysize;
		desc.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_VIDEOMEMORY;
		desc.ddsCaps.dwCaps2 = 0;

		// then create the scale surface
		result = IDirectDraw7_CreateSurface(ddraw7, &desc, &scale_surface, NULL);
		if (result != DD_OK)
		{
			verbose_printf(_WINDOWS("Error creating scale surface: %08x\n"), (UINT32)result);
			goto error_handling;
		}
	}
#endif /* USE_SCALE_EFFECTS */
	...
	// print out the good stuff
	{
		...
		verbose_printf(_WINDOWS("%s surface created: %s (%s)\n"), (texture_desc.ddsCaps.dwCaps2 & DDSCAPS2_TEXTUREMANAGE) ? _WINDOWS("Managed texture") : _WINDOWS("Texture"), size, colour);
		if (blit_surface != texture_surface)
		{
			print_surface_size(blit_surface, size);
			verbose_printf(_WINDOWS("Blit surface created: %s\n"), size);
		}
		if (preprocess_surface)
		{
			print_surface_size(preprocess_surface, size);
			verbose_printf(_WINDOWS("Pre-process texture surface created: %s\n"), size);
		}
#ifdef USE_SCALE_EFFECTS
		if (scale_surface)
		{
			print_surface_size(scale_surface, size);
			verbose_printf(_WINDOWS("scale effects surface created: %s\n"), size);
		}
#endif /* USE_SCALE_EFFECTS */
	}
...
static void set_gamma(void)
{
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("Warning: could not create gamma control to change brightness: %08x\n"), (UINT32)result);
		gamma_control = NULL;
	}
...
static void erase_surfaces(void)
{
	...
	if (scale_surface)
		result = IDirectDrawSurface7_Blt(scale_surface, NULL, NULL, NULL, DDBLT_COLORFILL | DDBLT_WAIT, &blitfx);
#endif /* USE_SCALE_EFFECTS */
...
tryagain:
	...
	if (result != DD_OK)
	{
		verbose_printf(_WINDOWS("Unable to lock blit_surface: %08x\n"), (UINT32)result);
		return 0;
	}
	...
	if (scale_effect.effect)
	{
		DDSURFACEDESC2 desc = { sizeof(DDSURFACEDESC2) };
		RECT scale_rect = { 0, 0,
		                    blit_width * effect_min_xscale * scale_effect.xsize,
		                    blit_height * effect_min_yscale * scale_effect.ysize };
		int depth = blit_desc.DUMMYUNIONNAMEN(4).ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount;
		if (depth == 16 && blit_desc.DUMMYUNIONNAMEN(4).ddpfPixelFormat.DUMMYUNIONNAMEN(3).dwGBitMask == 0x03E0)
			depth = 15;
		...
		if (win_d3d_tex_manage)
		{
			// attempt to lock the texture surface (we only need to lock a part of the surface)
			result = IDirectDrawSurface7_Lock(texture_surface, &scale_rect, &desc, DDLOCK_SURFACEMEMORYPTR | DDLOCK_WRITEONLY | DDLOCK_DISCARDCONTENTS | (wait_for_lock ? DDLOCK_WAIT : 0), NULL);
		}
		else
		{
			// attempt to lock the scale surface
			result = IDirectDrawSurface7_Lock(scale_surface, NULL, &desc, DDLOCK_SURFACEMEMORYPTR | (wait_for_lock ? DDLOCK_WAIT : 0), NULL);
		}
		if (result == DDERR_SURFACELOST)
			goto surface_lost;
		// if it was busy (and we're not throttling), just punt
		...
		scale_perform_scale((UINT8 *)blit_desc.lpSurface + dstxoffs * blit_desc.DUMMYUNIONNAMEN(4).ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount / 8, (UINT8 *)desc.lpSurface,
		                    blit_desc.DUMMYUNIONNAMEN(1).lPitch, desc.DUMMYUNIONNAMEN(1).lPitch,
		                    //blit_width, blit_height, depth, update);
		                    blit_width, blit_height + (scale_effect.ysize - 1), depth, update);

		// unlock the blit surface
		IDirectDrawSurface7_Unlock(blit_surface, NULL);

		if (win_d3d_tex_manage)
		{
			// unlock the texture surface
			IDirectDrawSurface7_Unlock(texture_surface, NULL);
		}
		else
		{
			// unlock the scale surface
			IDirectDrawSurface7_Unlock(scale_surface, NULL);

			// blit the image to the texture surface ourselves when texture management in't used
			result = IDirectDrawSurface7_BltFast(texture_surface, 0, 0, scale_surface, &scale_rect, DDBLTFAST_WAIT);
			if (result == DDERR_SURFACELOST)
				goto surface_lost;
			if (result != DD_OK)
			{
				// error, print the error and fall back
				verbose_printf(_WINDOWS("Unable to blt scale_surface to texture_surface: %08x\n"), (UINT32)result);
				return 0;
			}
		}
	}
	...
	if (!win_d3d_tex_manage)
	{
		...
		{
			// error, print the error and fall back
			verbose_printf(_WINDOWS("Unable to blt blit_surface to texture_surface: %08x\n"), (UINT32)result);
			return 0;
		}
	}
	...
	src.top = 0;
#ifdef USE_SCALE_EFFECTS
	src.right = dstxoffs + (blit_width * effect_min_xscale * scale_effect.xsize);
	src.bottom = blit_height * effect_min_yscale * scale_effect.ysize;
#else /* USE_SCALE_EFFECTS */
	...
	do {
		...
		else
		{
			...
			// center
			dst.left += (margins.right - margins.left - (dst.right - dst.left)) / 2;
			dst.top += (margins.bottom - margins.top - (dst.bottom - dst.top)) / 2;
			dst.right += dst.left;
			dst.bottom += dst.top;
		}
...
surface_lost:
	verbose_printf(_WINDOWS("Restoring surfaces\n"));
...
tryagain:
	...
	if ((win_d3d_use_prescale && (current_prescalex > 1 || current_prescaley > 1)) || win_d3d_use_feedback)
	{
		...
		{
			// error, print the error and fall back
			verbose_printf(_WINDOWS("Unable to render to preprocess texture: %08x\n"), (UINT32)result);
			return 0;
		}
	...
	{
		// error, print the error and fall back
		verbose_printf(_WINDOWS("Unable to render to back_surface: %08x\n"), (UINT32)result);
		return 0;
	}
	...
	if (win_window_mode)
	{
		...
		if (result != DD_OK)
		{
			...
			verbose_printf(_WINDOWS("Unable to blt back_surface to primary_surface: %08x\n"), (UINT32)result);
			return 0;
		}
	}
	...
	if (win_d3d_use_rgbeffect)
	{
		...
		{
			// error, print the error and fall back
			verbose_printf(_WINDOWS("Unable to blt RGB effect to back_surface: %08x\n"),	(UINT32)result);
			return 0;
		}
	}
...
static void set_texture_coordinates(LPRECT src, int texture_width, int texture_height)
{
...
#endif /* USE_SCALE_EFFECTS */
	{
		if (win_d3d_swapxy)
		{
			screen_vertex[2].tu = screen_vertex[3].tu = (float)(!win_d3d_flipy ? src->right : src->left) / texture_width;
			screen_vertex[0].tu = screen_vertex[1].tu = (float)(!win_d3d_flipy ? src->left : src->right) / texture_width;
			screen_vertex[1].tv = screen_vertex[3].tv = (float)(!win_d3d_flipx ? src->bottom : src->top) / texture_height;
			screen_vertex[0].tv = screen_vertex[2].tv = (float)(!win_d3d_flipx ? src->top : src->bottom) / texture_height;
		}
		else
		{
			screen_vertex[0].tu = screen_vertex[2].tu = (float)(win_d3d_flipx ? src->right : src->left) / texture_width;
			screen_vertex[1].tu = screen_vertex[3].tu = (float)(win_d3d_flipx ? src->left : src->right) / texture_width;
			screen_vertex[0].tv = screen_vertex[1].tv = (float)(win_d3d_flipy ? src->bottom : src->top) / texture_height;
			screen_vertex[2].tv = screen_vertex[3].tv = (float)(win_d3d_flipy ? src->top : src->bottom) / texture_height;
		}
	}
	else
	{
		if (win_d3d_swapxy)
		{
			screen_vertex[2].tu = screen_vertex[3].tu = !win_d3d_flipy ? (float)(src->right - src->left) / texture_width : 0;
			screen_vertex[0].tu = screen_vertex[1].tu = !win_d3d_flipy ? 0 : (float)(src->right - src->left)  / texture_width;
			screen_vertex[1].tv = screen_vertex[3].tv = !win_d3d_flipx ? (float)(src->bottom - src->top) / texture_height : 0;
			screen_vertex[0].tv = screen_vertex[2].tv = !win_d3d_flipx ? 0 : (float)(src->bottom - src->top) / texture_height;
		}
		else
		{
			screen_vertex[0].tu = screen_vertex[2].tu = win_d3d_flipx ? (float)(src->right - src->left) / texture_width : 0;
			screen_vertex[1].tu = screen_vertex[3].tu = win_d3d_flipx ? 0 : (float)(src->right - src->left) / texture_width;
			screen_vertex[0].tv = screen_vertex[1].tv = win_d3d_flipy ? (float)(src->bottom - src->top) / texture_height : 0;
			screen_vertex[2].tv = screen_vertex[3].tv = win_d3d_flipy ? 0 : (float)(src->bottom - src->top) / texture_height;
		}
	}
}
