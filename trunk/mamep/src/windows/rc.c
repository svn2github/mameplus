...
#ifdef WINUI
#undef _WINDOWS
static const char *_WINDOWS(const char *str)
{
	const char *p;

	if ((p = lang_message(UI_MSG_OSD0, str)) != str)
		return p;
...
int rc_register(struct rc_struct *rc, struct rc_option *option)
{
   ...
   if(i >= (rc->option_size-1))
   {
       ...
       struct rc_option *tmp = realloc(rc->option,
        (rc->option_size + ALLOC_OPTION_SIZE) * sizeof(struct rc_option));
#else
      struct rc_option *tmp = realloc(rc->option,
      ...
      rc->option = tmp;
#ifdef WINUI
      memset(rc->option + rc->option_size, 0, ALLOC_OPTION_SIZE *
#else
      memset(rc->option + rc->option_size, 0, BUF_SIZE *
#endif
         sizeof(struct rc_option));
#ifdef WINUI
      rc->option_size += ALLOC_OPTION_SIZE;
#else
   ...
   int priority, int continue_on_errors)
{
   ...
   fprintf(stderr, _WINDOWS("info: trying to parse: %s\n"), name);
   ...
   int priority, int continue_on_errors)
{
   ...
   while(fgets(buf, BUF_SIZE, f))
   {
	  ...
	  if (!arg)
	  {
		  fprintf(stderr, _WINDOWS("error: garbage \"%s\" on line %d of file: %s\n"),
...
static int real_rc_write_changes(struct rc_option *option, mame_file *f, const char *description,
	int (*changed)(struct rc_option *, void *), void *param)
{
	int i;

	if (description)
		mame_fprintf(f, "### %s ###\n", _WINDOWS(description));

	for (i = 0; option[i].type; i++)
	{
		int is_changed = !changed || changed(&option[i], param);

#ifndef VERBOSE_COMMENT
		if (!is_changed)
			continue;
#endif
		...
		{
		case rc_seperator:
			mame_fprintf(f, "\n### %s ###\n", _WINDOWS(option[i].name));
			break;

		case rc_link:
			if (real_rc_write_changes(option[i].dest, f, NULL, changed, param))
				return -1;
			break;
		...
		case rc_float:
			if (is_changed)
				...
				mame_fprintf(f, "# %-19s   ", option[i].name);
...
{
	return real_rc_write_changes(rc->option, f, description, NULL, NULL);
}

int osd_rc_write_changes(struct rc_struct *rc, mame_file *f, const char *description,
	int (*changed)(struct rc_option *, void *), void *param)
{
	return real_rc_write_changes(rc->option, f, description, changed, param);
}
   ...
   int priority, int (*arg_callback)(char *arg))
{
   ...
   for(i=1; i<argc; i++)
   {
      if(argv[i][0] == '-')
      {
         ...
         if((option = rc_get_option2(rc->option, argv[i] + start)))
         {
            ...
            else
            {
               ...
               if (rc_requires_arg[option->type])
               {
                  ...
                  if (i >= argc)
                  {
                     fprintf(stderr, _WINDOWS("error: %s requires an argument\n"), argv[i-1]);
                     return -1;
                  }
                  arg = argv[i];
               }
         ...
         {
            fprintf(stderr, _WINDOWS("error: unknown option %s\n"), argv[i]);
            return -1;
         }
   ...
   for(i=0; option[i].type; i++)
   {
      switch (option[i].type)
      {
         ...
         default:
            ...
            fprint_colums(f, buf,
               _WINDOWS((option[i].help)? option[i].help:"no help available"));
      }
   }
}
      ...
      case rc_file:
         {
            ...
            if(!f)
            {
               fprintf(stderr, _WINDOWS("error: couldn't open file: %s\n"), arg);
               return -1;
            }
            if (*(FILE **)option->dest)
   ...
   if (stat(name, &stat_buffer))
   {
	 ...
	 if (mkdir(name, 0775))
...
#endif
         {
            fprintf(stderr, _WINDOWS("Error creating dir %s"), name);
            perror(" ");
            return -1;
         }
      }
      else
      {
         ...
         fprintf(stderr, _WINDOWS("Error opening %s"), name);
         perror(" ");
         return -1;
      }
   }
