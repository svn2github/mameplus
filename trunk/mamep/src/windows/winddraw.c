...
#ifdef USE_SCALE_EFFECTS
static LPDIRECTDRAWSURFACE scale_surface;
#endif /* USE_SCALE_EFFECTS */
...
#ifdef USE_SCALE_EFFECTS
static DDSURFACEDESC scale_desc;
#endif /* USE_SCALE_EFFECTS */
...
static int blit_and_flip(LPDIRECTDRAWSURFACE src_surface, LPDIRECTDRAWSURFACE target_surface, LPRECT src, LPRECT dst, int update);
...
int win_ddraw_init(int width, int height, int depth, int attributes, const struct win_effect_data *effect)
{
	...
	if (scale_init())
	{
		fprintf(stderr, _WINDOWS("Error initialising software scale effects\n"));
		goto cant_init_scale;
	}
#endif /* USE_SCALE_EFFECTS */
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error creating DirectDraw: %08x\n"), (UINT32)result);
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error getting DirectDraw capabilities: %08x\n"), (UINT32)result);
		goto cant_get_caps;
	}
	...
	if (win_dd_hw_stretch && verbose)
		fprintf(stderr, _WINDOWS("Hardware stretching supported\n"));
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error setting cooperative level: %08x\n"), (UINT32)result);
...
#ifdef USE_SCALE_EFFECTS
	if (verbose && scale_effect.effect)
		fprintf(stderr, _WINDOWS("Using %s scale effect\n"), scale_effect.name);
#endif /* USE_SCALE_EFFECTS */
...
#ifdef USE_SCALE_EFFECTS
	if (depth == 16 && desc->ddpfPixelFormat.DUMMYUNIONNAMEN(3).dwGBitMask == 0x03E0 && scale_check(15))
		return DDENUMRET_OK;
	else if (scale_check(depth))
		return DDENUMRET_OK;
#endif /* USE_SCALE_EFFECTS */
...
#ifdef USE_SCALE_EFFECTS
	if (depth == 16 && desc->DUMMYUNIONNAMEN(4).ddpfPixelFormat.DUMMYUNIONNAMEN(3).dwGBitMask == 0x03E0 && scale_check(15))
		return DDENUMRET_OK;
	else if (scale_check(depth))
		return DDENUMRET_OK;
#endif /* USE_SCALE_EFFECTS */
...
#ifdef USE_SCALE_EFFECTS
	target_width *= scale_effect.xsize;
	target_height *= scale_effect.ysize;
#endif /* USE_SCALE_EFFECTS */
...
static int set_resolution(void)
{
	...
	if (!win_window_mode && (win_switch_res || win_switch_bpp))
	{
		...
		if (!win_switch_res || !win_switch_bpp)
		{
			...
			if (result != DD_OK)
			{
				fprintf(stderr, _WINDOWS("Error getting display mode: %08x\n"), (UINT32)result);
				goto cant_get_mode;
			}
		...
		if (result != DD_OK)
		{
			fprintf(stderr, _WINDOWS("Error enumerating modes: %08x\n"), (UINT32)result);
		...
		{
			if (best_refresh)
				fprintf(stderr, _WINDOWS("Best mode = %dx%dx%d @ %d Hz\n"), best_width, best_height, best_depth, best_refresh);
			else
				fprintf(stderr, _WINDOWS("Best mode = %dx%dx%d @ default Hz\n"), best_width, best_height, best_depth);
		}
		...
		if (best_width != 0)
		{
			...
			if (result != DD_OK)
			{
				fprintf(stderr, _WINDOWS("Error setting mode: %08x\n"), (UINT32)result);
				goto cant_set_mode;
			}
...
static int create_surfaces(void)
{
	...
	{
		if (verbose)
			fprintf(stderr, _WINDOWS("Error creating primary surface: %08x\n"), (UINT32)result);
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error getting primary surface desc: %08x\n"), (UINT32)result);
	...
	if (verbose)
		fprintf(stderr, _WINDOWS("Primary surface created: %dx%dx%d (R=%08x G=%08x B=%08x)\n"),
	...
	if (!win_window_mode && win_triple_buffer)
	{
		...
		if (result != DD_OK)
		{
			fprintf(stderr, _WINDOWS("Error getting attached back surface: %08x\n"), (UINT32)result);
			goto cant_get_back_surface;
		}
	}

#ifdef USE_SCALE_EFFECTS
	// disable scale effect if current depth is not supported
	{
		int depth = primary_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount;
		if (depth == 16 && primary_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(3).dwGBitMask == 0x03E0)
			depth = 15;

		if (scale_check(depth))
		{
			fprintf(stderr, _WINDOWS("Warning: scale effect %s is disabled.\n"), scale_effect.name);
	...
	{
		scale_desc = primary_desc;

		scale_desc.dwFlags = DDSD_WIDTH | DDSD_HEIGHT | DDSD_PIXELFORMAT | DDSD_CAPS;
		scale_desc.ddsCaps.dwCaps = DDSCAPS_SYSTEMMEMORY;

		if (win_dd_hw_stretch)
		{
			scale_desc = blit_desc;

			// fill in the differences
			scale_desc.dwWidth *= scale_effect.xsize;
			scale_desc.dwHeight *= scale_effect.ysize;
			scale_desc.dwHeight += scale_effect.ysize;
			scale_desc.ddsCaps.dwCaps = DDSCAPS_VIDEOMEMORY;
		}

		// then create the scale surface
		result = IDirectDraw_CreateSurface(ddraw, &scale_desc, &scale_surface, NULL);
		if (result != DD_OK)
		{
			if (verbose)
				fprintf(stderr, _WINDOWS("Error creating scale surface: %08x\n"), (UINT32)result);
			goto cant_create_scale;
		}

		// get a description of the scale surface
		result = IDirectDrawSurface_GetSurfaceDesc(scale_surface, &scale_desc);
		if (result != DD_OK)
		{
			fprintf(stderr, _WINDOWS("Error getting scale surface desc: %08x\n"), (UINT32)result);
			goto cant_get_scale_desc;
		}
	}

	if (verbose && scale_surface)
		fprintf(stderr, _WINDOWS("Scale surface created: %dx%dx%d (R=%08x G=%08x B=%08x)\n"),
				(int)scale_desc.dwWidth,
				(int)scale_desc.dwHeight,
				(int)scale_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount,
				(UINT32)scale_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(2).dwRBitMask,
				(UINT32)scale_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(3).dwGBitMask,
				(UINT32)scale_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(4).dwBBitMask);
#endif /* USE_SCALE_EFFECTS */
...
cant_get_scale_desc:
	if (scale_surface)
		IDirectDrawSurface_Release(scale_surface);
	scale_surface = NULL;
cant_create_scale:
#endif /* USE_SCALE_EFFECTS */
	if (blit_surface)
...
static int create_blit_surface(void)
{
...
#ifdef USE_SCALE_EFFECTS
	//blit_desc.dwHeight += (scale_effect.effect ? 1 : 0);
	blit_desc.dwHeight += (scale_effect.effect ? scale_effect.ysize - 1 : 0);
	blit_desc.ddsCaps.dwCaps = scale_effect.effect ? DDSCAPS_SYSTEMMEMORY : DDSCAPS_VIDEOMEMORY;
	...
	{
		if (verbose)
			fprintf(stderr, _WINDOWS("Error creating blit surface: %08x\n"), (UINT32)result);
		goto cant_create_blit;
	}
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error getting blit surface desc: %08x\n"), (UINT32)result);
	...
	// print out the good stuff
	if (verbose)
		fprintf(stderr, _WINDOWS("Blit surface created: %dx%dx%d (R=%08x G=%08x B=%08x)\n"),
...
static void set_brightness(void)
{
	...
	{
		if (verbose)
			fprintf(stderr, _WINDOWS("Warning: could not create gamma control to change brightness: %08x\n"), (UINT32)result);
		gamma_control = NULL;
	}
	...
	if (gamma_control)
	{
		...
		if (result != DD_OK)
			fprintf(stderr, _WINDOWS("Error setting gamma ramp: %08x\n"), (UINT32)result);
	}
}
...
static int create_clipper(void)
{
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error creating clipper: %08x\n"), (UINT32)result);
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error setting clipper hwnd: %08x\n"), (UINT32)result);
		goto cant_set_hwnd;
	}
	...
	if (result != DD_OK)
	{
		fprintf(stderr, _WINDOWS("Error setting clipper on primary surface: %08x\n"), (UINT32)result);
...
#ifdef USE_SCALE_EFFECTS
	// erase the scale surface
	if (scale_surface)
		result = IDirectDrawSurface_Blt(scale_surface, NULL, NULL, NULL, DDBLT_COLORFILL | DDBLT_WAIT, &blitfx);
#endif /* USE_SCALE_EFFECTS */
...
#ifdef USE_SCALE_EFFECTS
	LPDIRECTDRAWSURFACE temp_surface = blit_surface;
#endif /* USE_SCALE_EFFECTS */
...
tryagain:
	...
	{
		if (verbose)
			fprintf(stderr, _WINDOWS("Unable to lock blit_surface: %08x\n"), (UINT32)result);
		return 0;
	}
	...
	{
		int depth = dstdepth;
		if (depth == 16 && blit_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(3).dwGBitMask == 0x03E0)
			depth = 15;

		// attempt to lock the scale surface
		result = IDirectDrawSurface_Lock(scale_surface, NULL, &scale_desc, DDLOCK_SURFACEMEMORYPTR | wait_for_lock ? DDLOCK_WAIT : 0, NULL);
		if (result == DDERR_SURFACELOST)
			goto surface_lost;
		...
		scale_perform_scale((UINT8 *)blit_desc.lpSurface + dstxoffs * blit_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount / 8,
		                    (UINT8 *)scale_desc.lpSurface + dstxoffs *scale_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount / 8,
		                    blit_desc.DUMMYUNIONNAMEN(1).lPitch, scale_desc.DUMMYUNIONNAMEN(1).lPitch,
		                    //win_visible_width, win_visible_height, depth, update);
		                    win_visible_width * effect_min_xscale, win_visible_height * effect_min_yscale + (scale_effect.ysize - 1), depth, update);
		temp_surface = scale_surface;

		// unlock the scale surface
		IDirectDrawSurface_Unlock(scale_surface, NULL);
	}
#endif /* USE_SCALE_EFFECTS */
	...
	src.top = 0;
#ifdef USE_SCALE_EFFECTS
	src.right = dstxoffs + (win_visible_width * effect_min_xscale * scale_effect.xsize);
	src.bottom = win_visible_height * effect_min_yscale * scale_effect.ysize;
#else /* USE_SCALE_EFFECTS */
	...
	// blit and flip
#ifdef USE_SCALE_EFFECTS
	if (!blit_and_flip(temp_surface, target_surface, &src, &dst, update))
#else /* USE_SCALE_EFFECTS */
	if (!blit_and_flip(blit_surface, target_surface, &src, &dst, update))
#endif /* USE_SCALE_EFFECTS */
		return 0;
...
surface_lost:
	if (verbose)
		fprintf(stderr, _WINDOWS("Recreating surfaces\n"));
...
tryagain:
	// do the blit
	result = IDirectDrawSurface_Blt(target_surface, dst, src_surface, src, DDBLT_ASYNC, NULL);
	if (result == DDERR_SURFACELOST)
		goto surface_lost;
	if (result != DD_OK && result != DDERR_WASSTILLDRAWING)
	{
		...
		if (verbose)
			fprintf(stderr, _WINDOWS("Unable to blt target_surface: %08x\n"), (UINT32)result);
		return 0;
	}
	...
	if (!win_window_mode && back_surface && result != DDERR_WASSTILLDRAWING)
	{
		...
		{
			total += stop - start;
			ui_popup(_WINDOWS("Avg Flip = %d"), (int)(total / (count - 100)));
		}
#endif
	}
	return 1;
...
#ifdef USE_SCALE_EFFECTS
	static LPDIRECTDRAWSURFACE temp_surface;
#endif /* USE_SCALE_EFFECTS */
	LPDIRECTDRAWSURFACE target_surface;
	struct win_blit_params params;
...
tryagain:
...
#ifdef USE_SCALE_EFFECTS
	if (scale_effect.effect && xmult >= scale_effect.xsize && ymult >= scale_effect.ysize)
	{
		if (temp_surface != scale_surface)
			update = 1;

		xmult /= scale_effect.xsize;
		ymult /= scale_effect.ysize;

		temp_surface = scale_surface;
	}
	else
		temp_surface = target_surface;
#endif /* USE_SCALE_EFFECTS */
	...
	if (temp_surface == scale_surface)
	{
		...
		inner.left = outer.left + ((outer.right - outer.left) - (win_visible_width * xmult * scale_effect.xsize)) / 2;
		inner.top = outer.top + ((outer.bottom - outer.top) - (win_visible_height * ymult * scale_effect.ysize)) / 2;
		inner.right = inner.left + win_visible_width * xmult * scale_effect.xsize;
		inner.bottom = inner.top + win_visible_height * ymult * scale_effect.ysize;
	}
	else
#endif /* USE_SCALE_EFFECTS */
	{
...
#ifdef USE_SCALE_EFFECTS
	result = IDirectDrawSurface_Lock(temp_surface, NULL, &temp_desc, wait_for_lock ? DDLOCK_WAIT : 0, NULL);
#else /* USE_SCALE_EFFECTS */
	...
	{
		if (verbose)
			fprintf(stderr, _WINDOWS("Unable to lock target_surface: %08x\n"), (UINT32)result);
		return 0;
	}
	...
	if (temp_surface == scale_surface)
	{
		// if scale effect is enabled, disable old scanlines effect
		params.dstyscale	= ymult;
		params.dstyskip		= 0;
	}
#endif /* USE_SCALE_EFFECTS */
	...
	if (bounds && !update && !vector_dirty_pixels)
	{
		...
		if (temp_surface == scale_surface)
		{
			params.dstxoffs += (bounds->min_x - win_visible_rect.left) * xmult * scale_effect.xsize;
			params.dstyoffs += (bounds->min_y - win_visible_rect.top) * ymult * scale_effect.ysize;
		}
		else
#endif /* USE_SCALE_EFFECTS */
		{
	...
	if (temp_surface == scale_surface)
	{
		...
		static int prev_top;
		DDSURFACEDESC target_desc = { sizeof(target_desc) };
		int left = params.dstxoffs;
		int top = params.dstyoffs;
		int width = params.srcwidth * xmult;
		int height = params.srcheight * ymult;
		int depth = temp_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount;
		if (depth == 16 && temp_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(3).dwGBitMask == 0x03E0)
			depth = 15;

		// attempt to lock the target surface
		result = IDirectDrawSurface_Lock(target_surface, NULL, &target_desc, wait_for_lock ? DDLOCK_WAIT : 0, NULL);
		...
		{
			// unlock temp surface
			IDirectDrawSurface_Unlock(temp_surface, NULL);
		...
		// adjust position for target surface
		if (left + width * scale_effect.xsize >= target_desc.dwWidth)
			width = (target_desc.dwWidth - left - 1) / scale_effect.xsize;
		if (top + height * scale_effect.ysize >= target_desc.dwHeight)
			height = (target_desc.dwHeight - top - 1) / scale_effect.ysize;

		if (prev_left != left || prev_top != top)
		{
			update = 1;
			prev_left = left;
			prev_top = top;
		...
		scale_perform_scale((UINT8 *)temp_desc.lpSurface
		                             + left * temp_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount / 8
		                             + top * temp_desc.DUMMYUNIONNAMEN(1).lPitch,
		                    (UINT8 *)target_desc.lpSurface
		                             + left * target_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount / 8
		                             + top * target_desc.DUMMYUNIONNAMEN(1).lPitch,
		                    temp_desc.DUMMYUNIONNAMEN(1).lPitch, target_desc.DUMMYUNIONNAMEN(1).lPitch,
		                    width, height, depth, update);
