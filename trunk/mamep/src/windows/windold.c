...
int win_create_window(int width, int height, int depth, int attributes, double aspect)
{
	...
	{
		if (win_blit_effect)
			fprintf(stderr, _WINDOWS("Warning: non-hardware-accelerated blitting-effects engine enabled\n         use the -d3deffect option to enable hardware acceleration\n"));
	}
#if 0
	else
	{
		if (win_d3d_effects_in_use())
			fprintf(stderr, _WINDOWS("Warning: hardware-accelerated blitting-effects selected, but currently disabled\n         use the -direct3d option to enable hardware acceleration\n"));
	}
#endif
...
static void dib_draw_window(HDC dc, mame_bitmap *bitmap, const rectangle *bounds, void *vector_dirty_pixels, int update)
{
	...
	// set 16-bit bitmask for 5-5-5 and 5-6-5
	if (depth == 16)
	{
		LPDWORD bmiBitmasks = (LPDWORD)&video_dib_info->bmiColors;

		video_dib_info->bmiHeader.biCompression = BI_BITFIELDS;

		bmiBitmasks[0] = (255 >> win_color16_rsrc_shift) << win_color16_rdst_shift;
		bmiBitmasks[1] = (255 >> win_color16_gsrc_shift) << win_color16_gdst_shift;
		bmiBitmasks[2] = (255 >> win_color16_bsrc_shift) << win_color16_bdst_shift;
	}
...
int debugwin_init_windows(void)
{
	...
	sprintf(buf, "Debug: %s [%s]", (options.use_lang_list ? _LST(Machine->gamedrv->description) : Machine->gamedrv->description), Machine->gamedrv->name);
	MultiByteToWideChar(ui_lang_info[options.langcode].codepage, 0, buf, -1, title, sizeof title);
