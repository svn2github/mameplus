...
WRITE8_DEVICE_HANDLER( suna8_play_samples_w )
{
	...
	if( data )
	{
		if( ~data & 0x10 )
		{
			sample_start_raw(device, 0, &state->samplebuf[0x800*state->sample], 0x0800, 4000, 0);
		}
		else if( ~data & 0x08 )
		{
			state->sample &= 3;
			sample_start_raw(device, 0, &state->samplebuf[0x800*(state->sample+7)], 0x0800, 4000, 0);
		}
	}
}
