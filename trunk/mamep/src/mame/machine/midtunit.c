...
DRIVER_INIT( mktunit )
{
	...
	memory_install_readwrite8_handler(cpu_get_address_space(machine->cpu[1], ADDRESS_SPACE_PROGRAM), 0xfb9c, 0xfbc6, 0, 0, (read8_space_func)SMH_BANK(9), (write8_space_func)SMH_BANK(9));
...
static void init_nbajam_common(running_machine *machine, int te_protection)
{
	...
	/* sound chip protection (hidden RAM) */
	if (!te_protection)
		memory_install_readwrite8_handler(cpu_get_address_space(machine->cpu[1], ADDRESS_SPACE_PROGRAM), 0xfbaa, 0xfbd4, 0, 0, (read8_space_func)SMH_BANK(9), (write8_space_func)SMH_BANK(9));
	else
		memory_install_readwrite8_handler(cpu_get_address_space(machine->cpu[1], ADDRESS_SPACE_PROGRAM), 0xfbec, 0xfc16, 0, 0, (read8_space_func)SMH_BANK(9), (write8_space_func)SMH_BANK(9));
	memory_set_bankptr(machine, 9, auto_alloc_array(machine, UINT8, 0x80));
}
...
DRIVER_INIT( jdreddp )
{
	...
	/* sound chip protection (hidden RAM) */
	memory_install_readwrite8_handler(cpu_get_address_space(machine->cpu[1], ADDRESS_SPACE_PROGRAM), 0xfbcf, 0xfbf9, 0, 0, (read8_space_func)SMH_BANK(7), (write8_space_func)SMH_BANK(9));
	memory_set_bankptr(machine, 9, auto_alloc_array(machine, UINT8, 0x80));
