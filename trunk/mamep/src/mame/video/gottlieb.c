...
static TILE_GET_INFO( get_bg_tile_info )
{
	...
	if ((code & 0x80) == 0)
		SET_TILE_INFO(state->gfxcharlo, code, 0, 0);
	else
		SET_TILE_INFO(state->gfxcharhi, code, 0, 0);
}
...
VIDEO_START( gottlieb )
{
	...
	compute_resistor_weights(0,	255, -1.0,
			4, resistances, state->weights, 180, 0,
			4, resistances, state->weights, 180, 0,
			4, resistances, state->weights, 180, 0);
	state->transparent0 = FALSE;
	...
	state_save_register_global(machine, state->transparent0);
}
...
VIDEO_START( screwloo )
{
	...
	/* configure the background tilemap */
	state->bg_tilemap = tilemap_create(machine, get_screwloo_bg_tile_info, tilemap_scan_rows, 8, 8, 32, 32);
	tilemap_set_transparent_pen(state->bg_tilemap, 0);
	tilemap_set_scrolldx(state->bg_tilemap, 0, 318 - 256);
...
static void draw_sprites(running_machine *machine, bitmap_t *bitmap, const rectangle *cliprect)
{
	...
	for (offs = 0; offs < 256; offs += 4)
	{
		...
		int code = (255 ^ spriteram[offs + 2]) + 256 * state->spritebank;
