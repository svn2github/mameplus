...
{
	const UINT8 *prom = memory_region(machine, REGION_PROMS) + 0x400;
	int i;
...
{
	UINT8 *prombase = memory_region(screen->machine, REGION_PROMS);
	UINT8 *gfxbase = memory_region(screen->machine, REGION_GFX1);
	int x, y;
