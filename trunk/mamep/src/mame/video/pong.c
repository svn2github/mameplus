	...
	if( readinputport(0) & 1 )
	{
		if( STOP_G )
		{
			...
			if(	!serve_timer )                                                 /* SU 059 */
			{
				...
				timer_set(ATTOTIME_IN_USEC((int)(1.2*330000.0*4.7)), NULL, 0, serve_timer_cb);  /* SU 059 */
				...
				if( HVID && VVID )
				{
					...
					if( HRESET )  /* no hit and HBLANK starts ? */
					{
						...
						if( ATRACT && !score_sound_timer )                                  /* SU 059 */
						{
							...
							timer_set(ATTOTIME_IN_USEC((int)(1.2*220000.0*1.0)), NULL, 0, score_timer_cb);   /* SU 059 */
