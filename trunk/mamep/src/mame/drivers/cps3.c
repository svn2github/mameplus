...
{
	UINT32 *rom =  (UINT32*)decrypted_gamerom;//machine->region("user4")->base();
	if (cps3_altEncryption) rom = (UINT32*)machine->region("user4")->base();
...
static MACHINE_RESET( cps3 )
{
	...
	{
		 fastboot_timer = machine->scheduler().timer_alloc(FUNC(fastboot_timer_callback), NULL);
		 fastboot_timer->adjust(attotime::zero);
	}
