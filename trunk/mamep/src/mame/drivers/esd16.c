...
/**/AM_RANGE(0x300000, 0x3007ff) AM_READ(SMH_RAM				)	// Sprites
/**/AM_RANGE(0x400000, 0x403fff) AM_READ(SMH_RAM				)	// Layers
/**/AM_RANGE(0x420000, 0x423fff) AM_READ(SMH_RAM				)	//
/**/AM_RANGE(0x500000, 0x500003) AM_READ(SMH_RAM				)	// Scroll
...
static ADDRESS_MAP_START( multchmp_writemem, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x300000, 0x3007ff) AM_WRITE(SMH_RAM) AM_BASE(&spriteram16) AM_SIZE(&spriteram_size	)	// Sprites
	...
	AM_RANGE(0x500000, 0x500003) AM_WRITE(SMH_RAM) AM_BASE(&esd16_scroll_0	)	//
	AM_RANGE(0x500004, 0x500007) AM_WRITE(SMH_RAM) AM_BASE(&esd16_scroll_1	)	//
	AM_RANGE(0x500008, 0x50000b) AM_WRITE(SMH_RAM						)	// ? 0
	AM_RANGE(0x50000c, 0x50000f) AM_WRITE(SMH_RAM						)	// ? 0
...
static ADDRESS_MAP_START( tangtang_writemem, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x700000, 0x70ffff) AM_WRITE(SMH_RAM)
ADDRESS_MAP_END
...
ROM_START( multchmp )
	...
	ROM_REGION( 0x140000, "gfx1", ROMREGION_DISPOSE )	/* Sprites, 16x16x5 */
...
ROM_START( hedpanio ) /* Story line & game instructions in English, copyright year is 1999 */
	...
	ROM_REGION( 0x600000, "gfx1", ROMREGION_DISPOSE )	/* Sprites, 16x16x5 */
	ROM_LOAD( "ver615_esd6.ju01", 0x200000, 0x200000, BAD_DUMP CRC(5858372c) SHA1(dc96112587df681d53cf7449bd39477919978325) )
	ROM_LOAD( "ver615_esd7.ju02", 0x000000, 0x200000, BAD_DUMP CRC(055d525f) SHA1(85ad474691f96e47311a1904015d1c92d3b2d607) )
	/* expand this to take up 0x200000 bytes too so we can decode it */
	ROM_LOAD16_BYTE( "esd5.rom", 0x400000, 0x080000, CRC(82c5727f) SHA1(017f1d0c94475c51d17f12e24895f47a273a2dbb) )
	ROM_FILL(                    0x500000, 0x100000, 0 )

	ROM_REGION( 0x400000, "gfx2", ROMREGION_DISPOSE )	/* Layers, 16x16x8 */
	ROM_LOAD16_BYTE( "ver615_esd8.fu35", 0x000000, 0x200000, BAD_DUMP CRC(23aceb4f) SHA1(35d9ebc33b9e1515e47750cfcdfc0bf8bf44b71d) )
...
ROM_START( swatpolc ) /* SWAT Police */
	...
	ROM_REGION( 0x280000, "gfx1", ROMREGION_DISPOSE )	/* Sprites, 16x16x5 */
