	...
	switch (offset)
	{
		...
		case 0x08/8:
			if (ACCESSING_BITS_56_63)
			{
				...
				data >>= 56;
				data = (~data) & 0xf;
				memory_set_bankptr( 1, memory_region( machine, "user1" ) + 0x800000 + (data * 0x800000)); /* banked CROM */
			}
			if (ACCESSING_BITS_24_31)
			{
				data >>= 24;
...
ROM_START( lemans24 )	/* step 1.5 */
	...
	ROM_REGION( 0x1000000, "user3", ROMREGION_DISPOSE )  /* Video ROMs Part 1 */
	...
	ROM_REGION( 0x1000000, "user4", ROMREGION_DISPOSE )  /* Video ROMs Part 2 */
...
ROM_START( von2 )	/* Step 2.0 */
	...
	ROM_REGION( 0x2000000, "user4", ROMREGION_DISPOSE )  /* Video ROMs Part 2 */
...
static WRITE16_HANDLER( model3snd_ctrl )
{
	...
	if (memory_region_length(machine, "scsp2") > 0x800000)
	{
		UINT8 *snd = memory_region(machine, "scsp2");
		if (data & 0x20)
		{
...
GAME( 1998, dirtdvla,dirtdvls, model3_21, model3,   dirtdvls, ROT0, "Sega", "Dirt Devils (alt) (Revision A)", GAME_NOT_WORKING | GAME_IMPERFECT_GRAPHICS | GAME_NO_SOUND )
