...
static ADDRESS_MAP_START( zn_map, ADDRESS_SPACE_PROGRAM, 32 )
	...
	AM_RANGE(0x1faf0000, 0x1faf07ff) AM_READWRITE8(at28c16_0_r, at28c16_0_w, 0xffffffff) /* eeprom */
...
static MACHINE_RESET( coh1000c )
{
	...
	memory_set_bankptr( 3, memory_region( machine, "user3" ) ); /* country rom */
...
{
	memory_set_bankptr( 1, memory_region( machine, "user2" ) ); /* banked game rom */
...
{
	memory_set_bankptr( 1, memory_region( machine, "user2" ) ); /* GNET boot rom */
	zn_machine_init(machine);
}
...
{
	memory_set_bankptr( 1, memory_region( machine, "user2" ) ); /* banked rom */
	zn_machine_init(machine);
}
...
{
	memory_set_bankptr( 1, memory_region( machine, "user2" ) ); /* fixed game rom */
	memory_set_bankptr( 2, memory_region( machine, "user3" ) ); /* banked rom */
	zn_machine_init(machine);
}
...
ROM_START( cpzn1 )
	CPZN1_BIOS
	ROM_REGION( 0x50000, "audio", ROMREGION_ERASE00 )
ROM_END
...
ROM_START( glpracr )
	...
	ROM_REGION( 0x50000, "audio", 0 ) /* 64k for the audio CPU (+banks) */
...
ROM_START( 1on1gov )
	...
	ROM_REGION( 0x800, "user4", 0 ) /* at28c16 */
...
ROM_START( taitofx1 )
	TAITOFX1_BIOS
	ROM_REGION( 0x080000, "audio", ROMREGION_ERASE00 )
ROM_END
...
ROM_START( mgcldate )
	...
	ROM_REGION( 0x2c000, "audio", 0 )     /* 64k for Z80 code */
