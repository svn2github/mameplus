...
static sound_stream *lupin3_tone_stream;
static UINT32 lupin3_tone_step;
static UINT32 lupin3_tone_pos;

static void *lupin3_sh_start(int clock, const struct CustomSound_interface *config);

static const char *lupin3_sample_names[] =
{
	"*lupin3",
	"walk1.wav",	/* 0: player walk (low) */
	"walk2.wav",	/* 1: player walk (high) */
	"heli.wav",	/* 2: helicopter */
	"warp.wav",	/* 3: warp */
	"cap.wav",	/* 4: captured */
	"extend.wav",	/* 5: extend */
	"kick.wav",	/* 6: Fujiko kick */
	"bark.wav",	/* 7: dog's bark */
	0       /* end of array */
};

struct Samplesinterface lupin3_samples_interface =
{
	2,	/* 2 channels */
	lupin3_sample_names
};

struct CustomSound_interface lupin3_custom_interface =
{
	lupin3_sh_start
};
...
WRITE8_HANDLER( lupin3_sound_1_w )
{
	/*
		...
		bit 3:   captured
		bit 4:   extend
		bit 5:   unknown (bit on during game play)
		bit 6:   Fujiko kick
	*/
...
WRITE8_HANDLER( lupin3_tone_w )
{
	...
	if (data && data != 0xff)
	{
		double freq = 12050.0 / (256 - data);

		lupin3_tone_step = (UINT32)(freq * 65536.0 * 65536.0 / (double)Machine->sample_rate);
	}
}

static void lupin3_tone_update(void *param, stream_sample_t **inputs, stream_sample_t **outputs, int len)
{
	...
	if (lupin3_tone_step)
	{
		while (len-- > 0)
		{
			*buffer++ = lupin3_tone_pos & 0x80000000 ? 32767 : -32768;
			lupin3_tone_pos += lupin3_tone_step;
		}
	}
}
...
static void *lupin3_sh_start(int clock, const struct CustomSound_interface *config)
{
	...
	lupin3_tone_stream = stream_create(0, 1, Machine->sample_rate, NULL, lupin3_tone_update);
...
static sound_stream *ballbomb_tone_stream;
static UINT32 ballbomb_tone_step;
static UINT32 ballbomb_tone_pos;

static void *ballbomb_sh_start(int clock, const struct CustomSound_interface *config);

struct CustomSound_interface ballbomb_custom_interface =
{
	ballbomb_sh_start
};
...
WRITE8_HANDLER( ballbomb_tone_w )
{
	...
	if (data && data != 0xff)
	{
		double freq = ((6815.0 / (256 - (data|0x80) + (data<0x70 ? 1 : 0))) - (data==0x72 ? 13 : 0) - (data==0x73 ? 22 : 0))*0.42;

		ballbomb_tone_step = (UINT32)(freq * 65536.0 * 65536.0 / (double)Machine->sample_rate);
	}
}

static void ballbomb_tone_update(void *param, stream_sample_t **inputs, stream_sample_t **outputs, int len)
{
	stream_sample_t *buffer = outputs[0];

	memset(buffer, 0, len * sizeof(stream_sample_t));

	if (ballbomb_tone_step)
	{
		while (len-- > 0)
		{
			*buffer++ = ballbomb_tone_pos  & 0x80000000 ? 32767 : -32768;
			ballbomb_tone_pos += ballbomb_tone_step;
		}
	}
}
...
static void *ballbomb_sh_start(int clock, const struct CustomSound_interface *config)
{
	...
	ballbomb_tone_stream = stream_create(0, 1, Machine->sample_rate, NULL, ballbomb_tone_update);
