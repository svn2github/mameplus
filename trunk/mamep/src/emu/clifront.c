...
void cli_frontend::listfull(const char *gamename)
{
	...
	// print the header
	osd_printf_info(_("Name:             Description:\n"));
...
void cli_frontend::listclones(const char *gamename)
{
	...
	if (drivlist.count() == 0)
	{
		...
		if (original_count == 0)
			...
			osd_printf_info(_("Found %d matches for '%s' but none were clones\n"), drivlist.count(), gamename);
		return;
	}

	// print the header
	osd_printf_info(_("Name:            Clone of:\n"));
...
void cli_frontend::listbrothers(const char *gamename)
{
	...
	// print the header
	osd_printf_info(_("Source file:     Name:            Parent:\n"));
...
void cli_frontend::listroms(const char *gamename)
{
	...
	while (drivlist.next())
	{
		...
		if (!first)
			osd_printf_info("\n");
		first = false;
		osd_printf_info(_("ROMs required for driver \"%s\".\n"
			...
			for (const rom_entry *region = rom_first_region(*device); region; region = rom_next_region(region))
				for (const rom_entry *rom = rom_first_file(region); rom; rom = rom_next_file(rom))
				{
					...
					if (!hashes.flag(hash_collection::FLAG_NO_DUMP))
					{
						...
						osd_printf_info(_(" NO GOOD DUMP KNOWN"));
...
void cli_frontend::listsamples(const char *gamename)
{
	...
	while (drivlist.next())
	{
		...
		if (!first)
			osd_printf_info("\n");
		first = false;
		osd_printf_info(_("Samples required for driver \"%s\".\n"), drivlist.driver().name);
...
void cli_frontend::verifyroms(const char *gamename)
{
		...
		else
		{
			...
			switch (summary)
			{
				case media_auditor::INCORRECT:
					osd_printf_info(_("is bad\n"));
					incorrect++;
					break;

				case media_auditor::CORRECT:
					osd_printf_info(_("is good\n"));
					correct++;
					break;

				case media_auditor::BEST_AVAILABLE:
				case media_auditor::NONE_NEEDED:
					osd_printf_info(_("is best available\n"));
					correct++;
					break;
	...
	if (matched > 0 && correct == 0 && incorrect == 0)
	{
		if (notfound > 0)
			...
			throw emu_fatalerror(MAMERR_MISSING_FILES, _("romset \"%s\" has no roms!\n"), gamename);
	}
	...
	else
	{
		...
		osd_printf_info(_("%d romsets found, %d were OK.\n"), correct, correct);
	}
}
...
void cli_frontend::verifysamples(const char *gamename)
{
	...
	while (drivlist.next())
	{
		...
		else if (summary != media_auditor::NONE_NEEDED)
		{
			...
			// output the name of the driver and its clone
			osd_printf_info(_("sampleset %s "), drivlist.driver().name);
			int clone_of = drivlist.clone();
			if (clone_of != -1)
				osd_printf_info("[%s] ", drivlist.driver(clone_of).name);
	...
	// return an error if none found
	if (matched == 0)
		throw emu_fatalerror(MAMERR_NO_SUCH_GAME, _("No matching games found for '%s'"), gamename);
	...
	else
	{
		...
		osd_printf_info(_("%d samplesets found, %d were OK.\n"), correct, correct);
	}
}
...
void cli_frontend::romident(const char *filename)
{
	...
	osd_printf_info(_("Identifying %s....\n"), filename);
...
/*-------------------------------------------------
    listgames - output the manufacturer
...
void cli_frontend::listgames(const char *gamename)
{
	driver_enumerator drivlist(m_options, gamename);
	int count = 0;
	...
	while (drivlist.next())
	{
		if ((drivlist.driver().flags & GAME_NO_STANDALONE) == 0 && core_strwildcmp(gamename, drivlist.driver().name) == 0)
		{
			char name[200];

			osd_printf_info("%-5s%-36s ", drivlist.driver().year, _MANUFACT(drivlist.driver().manufacturer));

			if (lang_message_is_enabled(UI_MSG_LIST))
			{
				strcpy(name, _LST(drivlist.driver().description));
				osd_printf_info("\"%s\"\n", name);
				continue;
			}

			namecopy(name, drivlist.driver().description);
			osd_printf_info("\"%s",name);

			/* print the additional description only if we are listing clones */
			{
				const char *pdest = strstr(drivlist.driver().description, " (");

				if (pdest != NULL && pdest > drivlist.driver().description)
					osd_printf_info("%s", pdest);
			}
...
void cli_frontend::execute_commands(const char *exename)
{
	...
	{
		//FIXME for mamepgui
		if (!m_options.bool_value(OPTION_READCONFIG))
			lang_set_langcode(m_options, UI_LANG_EN_US);
...
void media_identifier::identify_file(const char *name)
{
	...
	if (core_filename_ends_with(name, ".chd"))
	{
		...
		if (!chd.compressed())
		{
			osd_printf_info(_("is a writeable CHD\n"));
			return;
		}
