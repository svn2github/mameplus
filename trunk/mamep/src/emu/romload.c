...
static void handle_missing_file(rom_load_data *romdata, const rom_entry *romp)
{
	...
	{
		astring_catprintf(romdata->errorstring, _("OPTIONAL %s NOT FOUND\n"), ROM_GETNAME(romp));
		romdata->warnings++;
	}
	...
	{
		astring_catprintf(romdata->errorstring, _("%s NOT FOUND (NO GOOD DUMP KNOWN)\n"), ROM_GETNAME(romp));
		romdata->warnings++;
	}
	...
	{
		astring_catprintf(romdata->errorstring, _("%s NOT FOUND\n"), ROM_GETNAME(romp));
		romdata->errors++;
	}
}
...
static void dump_wrong_and_correct_checksums(rom_load_data *romdata, const char *hash, const char *acthash)
{
	...
	astring_catprintf(romdata->errorstring, _("    EXPECTED: %s\n"), chksum);
	...
	astring_catprintf(romdata->errorstring, _("       FOUND: %s\n"), chksum);
...
static void verify_length_and_hash(rom_load_data *romdata, const char *name, UINT32 explength, const char *hash)
{
	...
	{
		astring_catprintf(romdata->errorstring, _("%s WRONG LENGTH (expected: %08x found: %08x)\n"), name, explength, actlength);
		romdata->warnings++;
	}
	...
	{
			astring_catprintf(romdata->errorstring, _("%s NO GOOD DUMP KNOWN\n"), name);
		romdata->warnings++;
	}
	/* verify checksums */
	else if (!hash_data_is_equal(hash, acthash, 0))
	{
		...
		astring_catprintf(romdata->errorstring, _("%s WRONG CHECKSUMS:\n"), name);
	...
	{
		astring_catprintf(romdata->errorstring, _("%s ROM NEEDS REDUMP\n"),name);
		romdata->warnings++;
	}
}
...
static void display_rom_load_results(rom_load_data *romdata)
{
	...
	if (romdata->warnings)
	{
		astring_catc(romdata->errorstring, _("WARNING: the "GAMENOUN" might not run correctly."));
		...
		if (ROMENTRY_ISFILE(romp))
		{
			...
			if (err != CHDERR_NONE)
			{
				if (err == CHDERR_FILE_NOT_FOUND)
					...
					astring_catprintf(romdata->errorstring, _("%s CHD ERROR: %s\n"), astring_c(filename), chd_error_string(err));
			...
			{
				astring_catprintf(romdata->errorstring, _("%s CHD NEEDS REDUMP\n"), astring_c(filename));
				romdata->warnings++;
			}
			...
			if (!DISK_ISREADONLY(romp))
			{
				...
				if (err != CHDERR_NONE)
				{
					astring_catprintf(romdata->errorstring, _("%s DIFF CHD ERROR: %s\n"), astring_c(filename), chd_error_string(err));
					romdata->errors++;
					continue;
				}
			}
