...
static void handle_missing_file(rom_load_data *romdata, const rom_entry *romp)
{
	...
	{
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("OPTIONAL %s NOT FOUND\n"), ROM_GETNAME(romp));
		romdata->warnings++;
	}
	...
	{
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%s NOT FOUND (NO GOOD DUMP KNOWN)\n"), ROM_GETNAME(romp));
		romdata->warnings++;
	}
	...
	{
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%s NOT FOUND\n"), ROM_GETNAME(romp));
		romdata->errors++;
	}
}
...
static void verify_length_and_hash(rom_load_data *romdata, const char *name, UINT32 explength, const char* hash)
{
	...
	{
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%s WRONG LENGTH (expected: %08x found: %08x)\n"), name, explength, actlength);
		romdata->warnings++;
	}
	...
	{
			sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%s NO GOOD DUMP KNOWN\n"), name);
		romdata->warnings++;
	}
	/* verify checksums */
	else if (!hash_data_is_equal(hash, acthash, 0))
	{
		...
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%s WRONG CHECKSUMS:\n"), name);
	...
	{
		sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%s ROM NEEDS REDUMP\n"),name);
		romdata->warnings++;
	}
}
...
static void display_loading_rom_message(const char *name, rom_load_data *romdata)
{
	...
	if (name != NULL && romdata->romstotal > 0 /* mamep: fixed crash in sms */)
...
static void display_rom_load_results(rom_load_data *romdata)
{
	...
	if (romdata->errors != 0)
	{
		...
		strcat(romdata->errorbuf, _("ERROR: required files are missing, the "GAMENOUN" cannot be run."));
	...
	if (romdata->warnings)
	{
		strcat(romdata->errorbuf, _("WARNING: the "GAMENOUN" might not run correctly."));
		...
		if (ROMENTRY_ISFILE(romp))
		{
			...
			if (err != CHDERR_NONE)
			{
				if (err == CHDERR_UNSUPPORTED_VERSION)
					sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%s UNSUPPORTED CHD VERSION\n"), astring_c(filename));
				else
					sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%s NOT FOUND\n"), astring_c(filename));
			...
			if (!DISK_ISREADONLY(romp))
			{
				...
				if (err != CHDERR_NONE)
				{
					if (err == CHDERR_UNSUPPORTED_VERSION)
						...
						sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)], _("%s: CAN'T OPEN DIFF FILE\n"), astring_c(filename));
					romdata->errors++;
					continue;
				}
			}
