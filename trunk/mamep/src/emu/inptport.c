...
static void input_port_postload(void)
{
	...
	for (portentry = Machine->input_ports; portentry->type != IPT_END; portentry++)
	{
		...
		else if (portentry->type != IPT_DIPSWITCH_SETTING && portentry->type != IPT_CONFIG_SETTING)
		{
			...
			if (portentry->type == IPT_TOGGLE_AUTOFIRE)
				autofire_toggle_port = &port_info[portnum];

#ifdef USE_CUSTOM_BUTTON
			else if (portentry->type >= IPT_CUSTOM1 && portentry->type < IPT_CUSTOM1 + custom_buttons)
				custom_button_info[portentry->player][portentry->type - IPT_CUSTOM1] = &port_info[portnum].bit[bitnum];
#endif /* USE_CUSTOM_BUTTON */
	...
	for (portnum = 0; portnum < MAX_INPUT_PORTS; portnum++)
	{
		...
		for (bitnum = 0, info = &portinfo->bit[0]; bitnum < MAX_BITS_PER_PORT && info->portentry; bitnum++, info++)
			if (input_port_condition(info->portentry))
			{
				...
				if (portentry->type >= IPT_CUSTOM1 && portentry->type < IPT_CUSTOM1 + MAX_CUSTOM_BUTTONS)
				{
					if (input_seq_pressed(input_port_seq(info->portentry, SEQ_TYPE_STANDARD)))
					{
						if (info->autopressed > autofiredelay[portentry->player])
							info->autopressed = 0;
	...
	if (portentry->type >= IPT_BUTTON1 && portentry->type < IPT_BUTTON1 + MAX_NORMAL_BUTTONS)
{
	...
	if (is_auto)
	{
		if (pressed)
		{
			if (info->autopressed > autofiredelay[portentry->player])
				info->autopressed = 0;
			else if (info->autopressed > autofiredelay[portentry->player] / 2)
				pressed = 0;
