...
static void parse_ini_file(const char *name)
{
	...
	options_set_string(mame_options(), OPTION_IPS, NULL, OPTION_PRIORITY_INI);
#endif /* USE_IPS */
...
/*-------------------------------------------------
    m68kcore_init - initialize the emulated machine
-------------------------------------------------*/
...
static void init_machine(running_machine *machine)
{
	...
	ui_set_startup_text(_("Initializing..."), TRUE);
...
static void handle_save(running_machine *machine)
{
	...
	if (timer_count_anonymous() > 0)
	{
		...
		{
			popmessage(_("Unable to save due to pending anonymous timers. See error.log for details."));
			goto cancel;
		}
		return;
	}
...
static void handle_load(running_machine *machine)
{
	...
	if (timer_count_anonymous() > 0)
	{
		...
		if (sub_mame_times(mame_timer_get_time(), mame->saveload_schedule_time).seconds > 0)
		{
			popmessage(_("Unable to load due to pending anonymous timers. See error.log for details."));
			goto cancel;
		}
		return;
	}
