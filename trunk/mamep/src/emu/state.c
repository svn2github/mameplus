...
{
	state_save_register_memory(module, tag, index, name, val->base, val->bpp / 8, val->rowpixels * val->height);
}
	...
	void (CLIB_DECL *errormsg)(const char *fmt, ...), const char *error_prefix)
{
	...
	{
		if (errormsg)
			errormsg(_("%sWrong version in save file (%d, 1 expected)"), error_prefix, header[8]);
		return -1;
	}
	...
	{
		if (errormsg)
			errormsg(_("%s'%s' is not a valid savestate file for game '%s'."), error_prefix, gamename);
		return -1;
	}
...
int state_save_load_begin(mame_file *file)
{
	...
	if (validate_header(ss_dump_array, NULL, get_signature(), popmessage, _("Error: ")))
	{
		free(ss_dump_array);
		ss_dump_array = NULL;
		ss_dump_size = 0;
