...
/********** CHEAT MENU **********/
static int		EnableDisableCheatMenu(running_machine *machine, int selection, int firstTime, int shortcut);
	...
	if(submenu_choice)
	{
		switch(sel)
		{
			case kMenu_EnableDisable:
				submenu_choice = EnableDisableCheatMenu(machine, submenu_choice, firstEntry, shortcut);
...
//      }
	...
	if(cheatListLength == 0)
	{
		...
		{
			menu_item[total] = _("there are no cheats for this game");
			menu_subitem[total] = NULL;
			flagBuf[total++] = 0;
		}
			...
			if((sel < (total - 1)) && entry)
			{
				...
				else
				{
					...
					else
					{
						...
						if(entry->flags & kCheatFlag_OneShot)
							ui_popup_time(1, _("%s activated"), entry->name);
					}
				}
			}
		}
...
static int EditCheatMenu(running_machine *machine, CheatEntry * entry, int index, int selection)
{
	...
	for(i = 0; i < entry->actionListLength; i++)
	{
		...
		if(i && isSelect)
		{
			...
			menuItem[total] = _("Link Extension");
...

		{
			...
			if(locationType == kLocation_Custom)
			{
				if(locationParameter == kCustomLocation_Comment)
				{
					...
					menuSubItem[total++] = _(kTypeNames[4]);
				}
				else
				{
					if(locationParameter == kCustomLocation_Select)
					{
						...
						menuSubItem[total++] = _(kTypeNames[5]);
					}
					else
						menuSubItem[total++] = _(kTypeNames[type & 3]);
				}
			}
			else
				menuSubItem[total++] = _(kTypeNames[type & 3]);
		...
		if(!wasCommentOrSelect)
		{
			if(type != kType_Watch)
			{
...

				{
					...
					menuItem[total] = _("Restore Previous Value");
			...
			if((type == kType_NormalOrDelay) || (type == kType_WaitForModification))
			{
				...
				if(	TEST_FIELD(traverse->type, OneShot) && TEST_FIELD(traverse->type, RestorePreviousValue) &&
					...
					menuItem[total] = _("Keep");
				else
					menuItem[total] = _("Delay");
			...
			if(type == kType_IgnoreIfDecrementing)
			{
				...
				menuItem[total] = _("Ignore Decrement By");
				menuSubItem[total++] = kNumbersTable[typeParameter];
			}
			...
			if(type == kType_Watch)
			{
...

				{
					...
					menuItem[total] = _("Watch Per Line");
...

				{
					...
					menuItem[total] = _("Watch Add Value");
			...
			else
			{
				...
				menuSubItem[total++] = _(kOperationNames[operation]);
			}
			...
			{
				{
					...
					menuItem[total] = _("Add/Subtract");
					menuSubItem[total++] = _(kAddSubtractNames[operationParameter]);
				}
				...
				if(locationType != kLocation_IndirectIndexed)
				{
					if(operationParameter)
					{
						...
						menuItem[total] = _("Minimum Boundary");
						menuSubItem[total++] = extendDataBuf[i];
					}
					else
					{
						...
						menuItem[total] = _("Maximum Boundary");
						menuSubItem[total++] = extendDataBuf[i];
					}
				}
			}
			...
			if((operation == kOperation_ForceRange) && (locationType != kLocation_IndirectIndexed))
			{
				...
				menuItem[total++] = _("Range Maximum");
			}
			...
			if(operation == kOperation_WriteMatch)
			{
				...
				menuItem[total] = _("Match to Write");
			...
			if((operation != kOperation_None) || (type == kType_Watch))
			{
				...
				if(type != kType_Watch)
				{
					...
					{
						{
							...
							menuItem[total] = _("Minimum Displayed Value");
...

						{
							...
							menuItem[total] = _("Minimum Value");
					...
					if(i && userSelect)
					{
						...
						menuItem[total] = _("Copy Previous Value");
...

				{
					...
					menuItem[total] = _("Byte Length");
					menuSubItem[total++] = _(kSizeNames[bytesUsed]);
				}
				...
				if(bytesUsed > 0)
				{
					...
					menuItem[total] = _("Endianness");
					menuSubItem[total++] = _(kEndiannessNames[EXTRACT_FIELD(traverse->type, Endianness)]);
				}

				{
					...
					menuItem[total] = _("Location");

					if(locationType == kLocation_Custom)
						menuSubItem[total++] = _(kCustomLocationNames[locationParameter]);
					else
						menuSubItem[total++] = _(kLocationNames[locationType]);
				}
				...
				if(locationType == kLocation_MemoryRegion)
				{
					...
					menuItem[total] = _("Region");
					menuSubItem[total++] = _(kRegionNames[locationParameter]);
				}
				...
				if(locationType == kLocation_IndirectIndexed)
				{
...

					{
						...
						menuItem[total] = _("Address Index");
						menuSubItem[total++] = extendDataBuf[i];
					}
				}
...
//          submenuChoice = 0;
	...
	{
		menu_item[total] = _(kOperandNameTable[search->lhs]);
		menu_subitem[total++] = NULL;

		menu_item[total] = _(kComparisonNameTable[search->comparison]);
		menu_subitem[total++] = NULL;

		menu_item[total] = _(kOperandNameTable[search->rhs]);
		menu_subitem[total++] = NULL;
	...
	{
		menu_item[total] = _("LHS");
		menu_subitem[total++] = _(kOperandNameTable[search->lhs]);

		menu_item[total] = _("Comparison");
		menu_subitem[total++] = _(kComparisonNameTable[search->comparison]);

		menu_item[total] = _("RHS");
		menu_subitem[total++] = _(kOperandNameTable[search->rhs]);

		menu_item[total] = _("Value");
	...
	menu_item[total] = _("Size");
	menu_subitem[total++] = _(kSearchByteNameTable[search->bytes]);

	menu_item[total] = _("Swap");
	menu_subitem[total++] = ui_getstring(search->swap ? UI_on : UI_off);

	menu_item[total] = _("Signed");
	menu_subitem[total++] = ui_getstring(search->sign ? UI_on : UI_off);

	sprintf(cpuBuffer, "%d", search->targetIdx);
	menu_item[total] = _("CPU");
	menu_subitem[total++] = cpuBuffer;

	menu_item[total] = _("Name");
	if(search->name)
		menu_subitem[total++] = search->name;
	else
		menu_subitem[total++] = _("(none)");

	menu_item[total] = "---";
	menu_subitem[total++] = NULL;

	menu_item[total] = _("Do Search");
	menu_subitem[total++] = NULL;

	menu_item[total] = _("Save Memory");
	menu_subitem[total++] = NULL;
...
static int EditWatch(running_machine *machine, WatchInfo * entry, int selection)
{
	...
	menuItem[total] = _("Length");
	menuSubItem[total] = buf[total];
	total++;

	menuItem[total] = _("Element Size");
	menuSubItem[total++] = _(kWatchSizeStringList[entry->elementBytes]);

	menuItem[total] = _("Label Type");
	menuSubItem[total++] = _(kWatchLabelStringList[entry->labelType]);

	menuItem[total] = _("Text Label");
	if(entry->label[0])
		menuSubItem[total++] = entry->label;
	else
		menuSubItem[total++] = _("(none)");

	menuItem[total] = _("Display Type");
	menuSubItem[total++] = _(kWatchDisplayTypeStringList[entry->displayType]);
	...
	menuItem[total] = _("Skip Bytes");
	menuSubItem[total] = buf[total];
	total++;

	sprintf(buf[total], "%d", entry->elementsPerLine);
	menuItem[total] = _("Elements Per Line");
	menuSubItem[total] = buf[total];
	total++;
...
static int SelectOptions(running_machine *machine, int selection)
{
	...
	/********** MENU CONSTRUCTION **********/
	menuItem[total] =		_("Select Search");
	menuSubItem[total++] =	NULL;

	menuItem[total] =		_("Search Dialog Style");
	switch(EXTRACT_FIELD(cheatOptions, SearchBox))
	{
		case kSearchBox_Minimum:
			menuSubItem[total++] = _("Minimum");
			break;

		case kSearchBox_Classic:
			menuSubItem[total++] = _("Classic");
			break;

		case kSearchBox_Advanced:
			menuSubItem[total++] = _("Advanced");
			break;
	}

	menuItem[total] =		_("Show Search Labels");
	menuSubItem[total++] =	ui_getstring(TEST_FIELD(cheatOptions, DontPrintNewLabels) ? UI_off : UI_on);

	menuItem[total] =		_("Auto Save Cheats");
	menuSubItem[total++] =	ui_getstring(TEST_FIELD(cheatOptions, AutoSaveEnabled) ? UI_on : UI_off);

	menuItem[total] =		_("Show Activation Key Message");
	menuSubItem[total++] =	ui_getstring(TEST_FIELD(cheatOptions, ActivationKeyMessage) ? UI_on : UI_off);

	menuItem[total] =		_("Load Old Format Code");
...
static void SetupCheatFromWatchAsWatch(CheatEntry * entry, WatchInfo * watch)
{
	if(watch && entry && watch->numElements)
	{
		...
		tempStringLength = sprintf(tempString, _("Watch %.8X (%d)"), watch->address, watch->cpu);
...
static void RestoreSearchBackup(SearchInfo * info)
{
	...
	if(info && info->backupValid)
	{
		...
		ui_popup_time(1, _("values restored"));
	}
	else
		ui_popup_time(1, _("there are no old values"));
}
	...
	switch(region->targetType)
	{
		case kRegionType_CPU:
		{
			...
			if(region->writeHandler)
			{
				...
				FPTR				handlerAddress = (FPTR)handler;
				if(	(handlerAddress >= ((FPTR)MWA8_BANK1)) && (handlerAddress <= ((FPTR)MWA8_BANK24)))
					sprintf(desc, _("BANK%.2d"), (int)(handlerAddress - (FPTR)MWA8_BANK1) + 1);
				else
				{
				...
				sprintf(desc, _("CPU%.2d "), region->targetIdx);
		...
		case kRegionType_Memory:
			sprintf(region->name, _("%.8X-%.8X MEMORY"), region->address, region->address + region->length - 1);
			break;

		default:
			sprintf(region->name, _("UNKNOWN"));
			break;
	}
}
...
static void ReloadCheatDatabase(running_machine *machine)
{
	...
	ui_popup_time(1, _("Cheat Database reloaded"));
}
