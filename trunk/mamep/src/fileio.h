...
#define SEARCHPATH_TRANSLATION		OPTION_TRANSLATION_DIRECTORY
#ifdef USE_HISCORE
#define SEARCHPATH_HISCORE_DB		NULL
#define SEARCHPATH_HISCORE		OPTION_HISCORE_DIRECTORY
#endif /* USE_HISCORE */
#ifdef USE_IPS
#define SEARCHPATH_IPS			OPTION_IPS_DIRECTORY
#endif /* USE_IPS */
...
/*-------------------------------------------------
    assemble_6_strings - allocate space for four
    strings and concatenate them into the new
    buffer
-------------------------------------------------*/

INLINE char *assemble_6_strings(const char *s1, const char *s2, const char *s3, const char *s4, const char *s5, const char *s6)
{
	char *tempbuf = malloc_or_die(strlen(s1) + strlen(s2) + strlen(s3) + strlen(s4) + strlen(s5) + strlen(s6) + 1);
	strcpy(tempbuf, s1);
	strcat(tempbuf, s2);
	strcat(tempbuf, s3);
	strcat(tempbuf, s4);
