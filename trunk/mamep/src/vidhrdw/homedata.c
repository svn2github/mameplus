...
VIDEO_UPDATE( mrokumei )
{
	...
	set_visible_area(0, 0*8, width*8-1, 2*8, 30*8-1);
...
VIDEO_UPDATE( pteacher )
{
	...
	if (blitter_bank & 0x04)
	{
		...
		{
			/* kludge for mjkinjas */
			set_visible_area(0, 0*8, 42*8-1, 2*8, 30*8-1);
			scroll_low = 0;
		}
		else
		{
			if (homedata_vreg[0x3] == 0xa6)
				set_visible_area(0, 0*8, 33*8-1, 2*8, 30*8-1);
			else
				set_visible_area(0, 0*8, 35*8-1, 2*8, 30*8-1);
	...
	{
		if (homedata_vreg[0x3] == 0xa6)
			set_visible_area(0, 0*8, 51*8-1, 2*8, 30*8-1);
		else
			set_visible_area(0, 0*8, 54*8-1, 2*8, 30*8-1);
