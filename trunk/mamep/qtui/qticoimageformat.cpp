...
void ICOReader::findColorInfo(QImage & image)
{
	...
	icoAttrib.blue_mask = 0x000000ff;
	icoAttrib.green_mask = 0x0000ff00;
	icoAttrib.red_mask = 0x00ff0000;
	icoAttrib.blue_shift = 0;
	icoAttrib.green_shift = 8;
	icoAttrib.red_shift = 16;
	icoAttrib.blue_scale = icoAttrib.green_scale = icoAttrib.red_scale = 1;
...
void ICOReader::readBMP(QImage & image)
{
	...
	read1BitBMP(image);
	if (icoAttrib.ncolors == 2 && qGray(image.color(0)) < qGray(image.color(1)))
	    qWarning("time to swap pixels");// swapPixel01( &image );		// pixel 0 is white!
...
void ICOReader::read16_24_23BMP(QImage & image)
{
    if (iod) {
	...
	register QRgb *p;
	QRgb  *end;
	uchar *buf24 = new uchar[image.bytesPerLine()];
	int    bpl24 = ((icoAttrib.w*icoAttrib.nbits+31)/32)*4;
	uchar *b;
	int c;
	...
	while (--h >= 0) {
	    ...
	    if (iod->read((char *)buf24,bpl24) != bpl24) {
		image = QImage();
		break;
	    }
	    b = buf24;
		...
		if (icoAttrib.nbits != 16)
		    c |= *(uchar*)(b+2)<<16;
		*p++ = qRgb(((c & icoAttrib.red_mask) >> icoAttrib.red_shift) * icoAttrib.red_scale,
		    ((c & icoAttrib.green_mask) >> icoAttrib.green_shift) * icoAttrib.green_scale,
		    ((c & icoAttrib.blue_mask) >> icoAttrib.blue_shift) * icoAttrib.blue_scale);
		b += icoAttrib.nbits/8;
	    }
	}
    ...
    if (count() > index) { // forces header to be read
        ...
        if (readIconEntry(index, &iconEntry)) {
            ...
            if (readBMPHeader(iconEntry, &header)) {
                ...
                if (icoAttrib.ncolors == 2) format = QImage::Format_Mono;
                else if (icoAttrib.ncolors > 0) format = QImage::Format_Indexed8;
...
bool ICOReader::write(QIODevice * device, const QList<QImage> & images)
{
    ...
    if (images.count()) {
        ...
        for (int i=0; i<id.idCount; i++) {
            ...
            int	nbits = 24;
            int	bpl_bmp = ((image.width()*24+31)/32)*4;
            ...
            maskImage.invertPixels(); // seams as thought it needs this
                                        // NOTE! !! The mask is only inverted vertically - not horizontally !!
