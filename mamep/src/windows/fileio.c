...
static int set_inipath(struct rc_option *option, const char *arg, int priority)
{
	pathlist[FILETYPE_INI].rawpath = strdup(arg);
...
#else /* !WINUI */
int (*osd_display_loading_rom_message_)(const char *name, rom_load_data *romdata);

#ifndef DONT_USE_DLL
#ifndef _MSC_VER
int osd_display_loading_rom_message(const char *name, rom_load_data *romdata)
{
	return osd_display_loading_rom_message_(name, romdata);
}
#endif /* !_MSC_VER */
#endif /* !DONT_USE_DLL */
