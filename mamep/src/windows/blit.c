...
static void check_for_mmx(void)
{
	...
	if (use_sse2 && verbose)
		fprintf(stderr, _WINDOWS("SSE2 supported\n"));
	else if (use_sse && verbose)
		fprintf(stderr, _WINDOWS("SSE supported\n"));
	else if (use_mmx && verbose)
		fprintf(stderr, _WINDOWS("MMX supported\n"));
	else if (verbose)
		fprintf(stderr, _WINDOWS("MMX not supported\n"));
}
