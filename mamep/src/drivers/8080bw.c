...
static ADDRESS_MAP_START( lupin3_writeport, ADDRESS_SPACE_IO, 8 )
	AM_RANGE(0x00, 0x00) AM_WRITE(lupin3_tone_w)
	AM_RANGE(0x02, 0x02) AM_WRITE(c8080bw_shift_amount_w)
	AM_RANGE(0x03, 0x03) AM_WRITE(lupin3_sound_1_w)
	AM_RANGE(0x04, 0x04) AM_WRITE(c8080bw_shift_data_w)
	AM_RANGE(0x05, 0x05) AM_WRITE(lupin3_sound_2_w)
	AM_RANGE(0x06, 0x06) AM_WRITE(watchdog_reset_w)
ADDRESS_MAP_END
...
static MACHINE_DRIVER_START( lupin3 )
	...
	MDRV_CPU_MODIFY("main")
	MDRV_CPU_PROGRAM_MAP(schaser_readmem,schaser_writemem)
	MDRV_CPU_IO_MAP(c8080bw_readport,lupin3_writeport)
	...
	MDRV_SOUND_CONFIG(lupin3_samples_interface)
	MDRV_SOUND_ROUTE(ALL_OUTPUTS, "mono", 0.25)

	MDRV_SOUND_ADD(CUSTOM, 0)
	MDRV_SOUND_CONFIG(lupin3_custom_interface)
...
static ADDRESS_MAP_START( ballbomb_writeport, ADDRESS_SPACE_IO, 8 )
	AM_RANGE(0x01, 0x01) AM_WRITE(ballbomb_tone_w)
	AM_RANGE(0x02, 0x02) AM_WRITE(c8080bw_shift_amount_w)
	AM_RANGE(0x04, 0x04) AM_WRITE(c8080bw_shift_data_w)
//	AM_RANGE(0x06, 0x06) AM_WRITE(c8080bw_shift_amount_w) //??? cloud drawing?
ADDRESS_MAP_END
...
static MACHINE_DRIVER_START( ballbomb )
	...
	MDRV_CPU_MODIFY("main")
	MDRV_CPU_PROGRAM_MAP(schaser_readmem,schaser_writemem)
	MDRV_CPU_IO_MAP(c8080bw_readport,ballbomb_writeport)
	...
	MDRV_SOUND_ADD(CUSTOM, 0)
	MDRV_SOUND_CONFIG(ballbomb_custom_interface)
...
/* Taito games */
	  ...
	  GAMEX(1979, schasrcv, schaser,  schasrcv, schasrcv, schaser,  ROT270, "Taito", "Space Chaser (CV version)", GAME_NO_SOUND | GAME_IMPERFECT_COLORS | GAME_NO_COCKTAIL )
