...
#include "osinline.h"
#else
#define direct_rgb_rshift	rshift
#define direct_rgb_gshift	gshift
#define direct_rgb_bshift	bshift
...
void artwork_update_video_and_audio(mame_display *display)
{
	...
	if (display->changed_flags & GAME_BITMAP_CHANGED)
	{
		...
		if (
			...
			options.use_transui ||
#endif /* TRANS_UI */
			last_uibounds.min_x != uibounds.min_x || last_uibounds.min_y != uibounds.min_y ||
...
#ifdef TRANS_UI
/*-------------------------------------------------
	artwork_fillbitmap_ts - draw the transparent
	box on the UI bitmap
-------------------------------------------------*/

/* fill a bitmap by transparent pen */
void artwork_fillbitmap_ts(mame_bitmap *dest,const rectangle *clip)
{
	int sx,sy,ex,ey,y;
...
#ifdef TRANS_UI
	int *yy = Machine->ui_orientation & ORIENTATION_SWAP_XY ? &x : &y;
#endif /* TRANS_UI */
			...
			for (h = 0; h < MAX_HINTS_PER_SCANLINE && hint[h] != 0; h++)
			{
				...
				for (x = start; x <= stop; x++)
				{
					...
					else
					{
						...
						draw_transparent32_RGB32(p, p, *yy);
					}
#endif /* TRANS_UI */
				}
			}
